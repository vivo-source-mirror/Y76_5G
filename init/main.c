...
//vivo lishaoxiong add for AT command begin
...
unsigned int recoverymode;
EXPORT_SYMBOL(recoverymode);

unsigned int update_firmware;
...
//vivo lishaoxiong add for AT command end
...
static void __init is_at_boot(char *command_line)
{
	...
	char *is_datadump_mode = "datadump_mode_flag=1";
	...
	if (strnstr(command_line, "datadump_mode_flag=1", strlen(command_line)) == NULL) {
		datadump_mode = 0;
	} else {
		datadump_mode = 1;
	}
