...
extern struct mtk_base_afe *afe_base;
static int i2s_enable_control;
static void mtk_i2s3_config_for_calibration(struct mtk_base_afe *afe, bool enable);
static const char *const mt6833_spk_type_str[] = {MTK_SPK_NOT_SMARTPA_STR,
						  MTK_SPK_RICHTEK_RT5509_STR,
						  MTK_SPK_MEDIATEK_MT6660_STR,
						  MTK_SPK_NXP_TFA98XX_STR,
						  MTK_SPK_TI_TAS2562_STR,
...
static const char *const i2s_enable_str[] = {"Off", "ON"};
static const struct soc_enum mtk_i2s_enum[] = {
		SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(i2s_enable_str),
			    i2s_enable_str),
};
...
static int mtk_i2s_enable_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s: i2s_enable_control=%d.\n", __func__, i2s_enable_control);
	ucontrol->value.integer.value[0] = i2s_enable_control;

	return 0;
}
static int mtk_i2s_enable_set(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	int enable = ucontrol->value.integer.value[0];
	mtk_i2s3_config_for_calibration(afe_base, enable);
	i2s_enable_control = enable;
	pr_info("%s: i2s_enable_control=%d.\n", __func__, i2s_enable_control);
...
static const struct snd_kcontrol_new mt6833_mt6359_controls[] = {
	...
	SOC_ENUM_EXT("Vivo MI2S Clock", mtk_i2s_enum[0],
		     mtk_i2s_enable_get, mtk_i2s_enable_set),
};
...
static void mtk_i2s3_config_for_calibration(struct mtk_base_afe *afe, bool enable)
{
	...
	if (enable) {
		...
		/* Config I2S3 gpio */
		mt6833_afe_gpio_request(afe, true, MT6833_DAI_I2S_3, 0);

		/* Config I2S3 : 48K, 32bit, non-HD */
		i2s_con = 0xA << I2S4_OUT_MODE_SFT;
		i2s_con |= 0x1 << I2S4_FMT_SFT;
		i2s_con |= 0x1 << I2S4_WLEN_SFT;
		i2s_con |= 0x1 << I2S4_HD_EN_SFT;
		regmap_update_bits(afe->regmap, AFE_I2S_CON3, 0xfffffffe, i2s_con);

		/* Enable I2S3 */
		regmap_update_bits(afe->regmap, AFE_I2S_CON3, I2S4_EN_MASK_SFT, 0x1);
		...
		/* Disable I2S3 */
		regmap_update_bits(afe->regmap, AFE_I2S_CON3, I2S4_EN_MASK_SFT, 0x0);
...
static struct tfa98xx_calib *tfa_calibration;
...
static int mt6833_mt6359_dev_probe(struct platform_device *pdev)
{
	...
	if (ret)
		dev_err(&pdev->dev, "%s snd_soc_register_card fail %d\n",
			__func__, ret);
#ifdef CONFIG_SND_CODEC_TFA9894
	tfa_calibration = get_tfa_cal();
	if (tfa_calibration && (0 == is_atboot)) {
		mtk_i2s3_config_for_calibration(afe_base, true);
		if (ret) {
			dev_err(&pdev->dev, "smartpa i2s clock failed\n");
			tfa_calibration->mi2s_ready = 1;
		} else {
			dev_info(&pdev->dev, "smartpa calibration start\n");
			tfa_calibration->mi2s_ready = 0;
			msleep(50);
			tfa_calibration->calibrate(false);
		}
		mtk_i2s3_config_for_calibration(afe_base, false);
		dev_info(&pdev->dev, "smartpa calibration done\n");
	}
#endif
