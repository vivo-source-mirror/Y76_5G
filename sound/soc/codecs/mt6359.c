			       ...
			       int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		regmap_update_bits(priv->regmap, MT6359_AUDENC_ANA_CON15,
				   ...
				   MIC_BIAS_2P1 << RG_AUDMICBIAS0VREF_SFT);
			       ...
			       int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		regmap_update_bits(priv->regmap, MT6359_AUDENC_ANA_CON17,
				   ...
				   MIC_BIAS_2P1 << RG_AUDMICBIAS2VREF_SFT);
		/* vow low power select */
		regmap_update_bits(priv->regmap, MT6359_AUDENC_ANA_CON17,
...
/*******************************************************************************
* mt_soc_ana register dump proc
* #ls -l
* -r--r--r-- 1 root system 0 2010-01-01 08:04 /proc/mtksocanaaudio
******************************************************************************/
static void __init mt6359_proc_dump_register(struct mt6359_priv *priv)
{
	...
	entry = proc_create_data("mtksocanaaudio", 0664, NULL, &mt6359_debugfs_ops, priv);
	if (!entry)
		dev_err(priv->dev, "%s() Can not create mtksocanaaudio\n", __func__);
	else
...
#endif
	mt6359_proc_dump_register(priv);
