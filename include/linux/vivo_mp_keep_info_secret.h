...
static char app_black_list[4][48] = {                   // sometimes needed for 'self-contradictory' restrictions
#ifdef CONFIG_BBK_FOR_RW
	"com.android.settings",
	"com.iqoo.secure",
	"com.vivo.devicereg",
#else
...
#endif
	for (i = 0; i < (int) (sizeof(app_black_list)/sizeof(app_black_list[0])) && app_black_list[i][0]; ++i) {
		if (strcmp(app_name, app_black_list[i]) == 0) {
			should = 1;
			break;
		}
	}
