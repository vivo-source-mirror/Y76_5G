...
static bool have_do_drop_filecache;
/*see fs/drop_caches.c drop_pagecache_sb()*/
static void vivo_drop_pagecache_sb(struct super_block *sb, void *unused)
{
...
static bool can_drop_file_cache(void)
{
	unsigned long inactive_file, file_mem;
	inactive_file = global_node_page_state(NR_LRU_BASE + LRU_FILE);
	file_mem = global_node_page_state(NR_FILE_PAGES) -
					global_node_page_state(NR_SHMEM) -
					global_node_page_state(NR_UNEVICTABLE) -
					total_swapcache_pages();

	/*check inactive_file > 512MB or file_mem > 800MB*/
	if ((inactive_file > 131072) || (file_mem > 204800))
		return true;
...
retry:
	...
	if (costly_order && !(gfp_mask & __GFP_RETRY_MAYFAIL)) {
		if (((!have_do_drop_filecache && (jiffies-INITIAL_JIFFIES)/CONFIG_HZ) < 100) &&
			can_drop_file_cache()) {
			u64 drop_start, drop_end;
			drop_start = local_clock();
			iterate_supers(vivo_drop_pagecache_sb, NULL);
			drop_end = local_clock();
			have_do_drop_filecache = true;
			count_vm_event(DROP_PAGECACHE);
			page = get_page_from_freelist(gfp_mask, order, alloc_flags, ac);
			pr_info("alloc order %d failed, drop file cache and got page %s, cost time %lu us",
				order, page ? "success" : "failed", (drop_end - drop_start)/1000);
			if (page) {
				goto got_pg;
			} else {
				goto nopage;
	...
	if (alloc_flags & ALLOC_WMARK_LOW) {
		if ((alloc_seq >= rsc_mem_pri_slowpath_bg_try_cnt) ||
			!atomic_read(&rsc_cgroup_cnt[RSC_CGROUP_TOPAPP]))
			alloc_flags &= ~ALLOC_WMARK_LOW;
	} else if (alloc_flags & ALLOC_WMARK_RBACK) {
		if ((alloc_seq >= rsc_mem_pri_slowpath_def_try_cnt) ||
			!atomic_read(&rsc_cgroup_cnt[RSC_CGROUP_TOPAPP]))
			alloc_flags &= ~ALLOC_WMARK_RBACK;
	}
#endif
