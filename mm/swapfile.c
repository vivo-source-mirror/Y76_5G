...
retry:
	...
	while ((i = find_next_to_unuse(si, i, frontswap)) != 0) {
		...
		if (swap_count(*swap_map) != SWAP_MAP_SHMEM)
			continue;
		...
		/*
		 * Don't hold on to prev_mm if it looks like exiting.
		 */
		if (atomic_read(&prev_mm->mm_users) == 1) {
			mmput(prev_mm);
			prev_mm = &init_mm;
			mmget(&init_mm);
		}
...
SYSCALL_DEFINE1(swapoff, const char __user *, specialfile)
{
	...
	pr_info("%s %d, <%s> <%d>, p->type %d p->pages %d, nr_swap_pages %d\n",
		__func__, __LINE__, current->comm, current->pid, p->type, p->pages, nr_swap_pages);
	total_swap_pages -= p->pages;
	p->flags &= ~SWP_WRITEOK;
	spin_unlock(&p->lock);
	spin_unlock(&swap_lock);
