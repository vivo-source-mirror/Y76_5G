...
struct mtk_panel_funcs {
	 ...
	 * @doze_suspend_active:
	 *
	 * doze suspend change to doze mode
	 */
	int (*doze_suspend_active)(struct drm_panel *panel,
		void *dsi_drv, dcs_write_gce cb, void *handle);

	/**
	 * @smart_aod_doze_area_set:
	 *
	 * set doze area.
	 */
	int (*smart_aod_doze_area_set)(struct drm_panel *panel,
		void *dsi_drv, dcs_write_gce cb, void *handle);
	/**
	 * @doze_get_mode_flags:
	 *
	...
	/*set dsi cmd by cmdq*/
	int (*set_dsi_cmd_cmdq)(void *dsi_drv, dcs_write_gce cb,
		void *handle, unsigned int type);

	/*get lcm id*/
	void (*lcm_get_id)(struct drm_panel *panel, int type);

	/*power control for incell panel for tp*/
	void (*dsi_panel_reset_and_powerctl)(struct drm_panel *panel, int enable);

	int (*hbm_set_cmdq)(struct drm_panel *panel, void *dsi_drv,
			    dcs_write_gce cb, void *handle, int en);
	void (*hbm_get_state)(struct drm_panel *panel, int *state);
	 ...
	 * @suspend_by_hs_mode:
	 *
	 * Call the @suspend_by_hs_mode before panel power off
	 * For smart aod panel
	 */
	int (*suspend_by_hs_mode)(struct drm_panel *panel,
		void *dsi_drv, dcs_write_gce cb, void *handle);
};
