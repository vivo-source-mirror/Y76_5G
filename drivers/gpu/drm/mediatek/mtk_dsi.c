	...
	int force_lcm_update)
{
	...
	if (dsi->panel && (!new_doze_state || force_lcm_update)) {
		if (dsi->ext && dsi->ext->funcs && dsi->ext->funcs->suspend_by_hs_mode) {
			dsi->ext->funcs->suspend_by_hs_mode(dsi->panel, dsi, mipi_dsi_dcs_write_gce2, NULL);
		} else {
			DDPINFO("panel_suspend_by_hs_mode function is NULL\n");
		}
