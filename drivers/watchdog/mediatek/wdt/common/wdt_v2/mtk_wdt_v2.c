...
static int mtk_wdt_probe(struct platform_device *dev)
{
	...
	/*
	 * add by vsen team add for vibrating for POWER_OFF_CHARGING_BOOT mode,
	 * need to check NONRST2 bit[23] for new platform.
	 */
	//0.boot mode
	boot_mode = get_boot_mode();
	pr_err("VSEN boot_mode = %d\n", boot_mode);
	//1.set bit [23]
	if (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
		|| boot_mode == LOW_POWER_OFF_CHARGING_BOOT) {
			reg = __raw_readl(MTK_WDT_NONRST_REG2);
			pr_err("VSEN read MTK_WDT_NONRST_REG2 = %d\n", reg);
			reg |= 1<<23;
			mt_reg_sync_writel(reg, MTK_WDT_NONRST_REG2);
			pr_err("VSEN write MTK_WDT_NONRST_REG2 = %d\n", reg);
	}
