...
#define VIVO_VENDOR_CPU_SET_LEN 32
...
struct board_info_ext {
	...
	char cpu_set[VIVO_VENDOR_CPU_SET_LEN];
} *boardinfo_ext;

extern char *get_board_version(void);
char board_version[12] = { 0 };
/*
...
static ssize_t vivo_show_cpu_set(struct boardinfo_obj *dev, struct boardinfo_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s\n", boardinfo_ext->cpu_set);
}
...
static int vivo_boardinfo_ext_init(void)
{
	...
	strlcpy(board_version, get_board_version(), sizeof(board_version));

	boardinfo_ext->core_num = num_possible_cpus();

	strlcpy(boardinfo_ext->cpu_type, "MT6853V", VIVO_CPU_TYPE_LEN);
	strlcpy(boardinfo_ext->vendor, "vivo", VIVO_VENDOR_LEN);
	strlcpy(boardinfo_ext->user_cpu_freq, "2.4", VIVO_VENDOR_LEN);
	strlcpy(boardinfo_ext->project_name, "PD2031EA", VIVO_VENDOR_PROJECT_NAME);
...
static int board_info_parse_dt(struct platform_device *pdevboard)
{
	...
	ret = of_property_read_string(board_node, "vivo,vendor_cpu_set", &temp_boardbuf);
	if (ret)
		pr_err("vivo,vendor_cpu_set property not defined\n");
	else
		strlcpy(boardinfo_ext->cpu_set, temp_boardbuf, VIVO_VENDOR_CPU_SET_LEN);
...
static struct boardinfo_attribute vivo_cpu_set_attribute =
	__ATTR(cpu_set, S_IRUGO, vivo_show_cpu_set,  NULL);
