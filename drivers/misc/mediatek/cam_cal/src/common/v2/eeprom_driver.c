...
//vivo lishuo modify for otp remake 20210823 [B210820-3030] begin
    struct stCAM_CAL_LIST_STRUCT *plist = NULL;
    unsigned int size_limit = 0;
    if(pdata->pdrv->slave_addr ==0){
        plist = get_list(&pdata->sensor_info);   
        size_limit = (plist && plist->maxEepromSize > 0)
		? plist->maxEepromSize : DEFAULT_MAX_EEPROM_SIZE_8K;
    }
    else{
        size_limit = pdata->pdrv->otp_MaxAddr;
    }  
	if (offset + size > size_limit) {
		pr_debug("Error! not support address >= 0x%x %d %d!!\n", size_limit,offset,size);
		return 0;
	}
	...
	if (plist && plist->readCamCalData) {
        ...
        dts_addr = pdata->pdrv->pi2c_client->addr;
        if(pdata->pdrv->slave_addr != 0){  
            pdata->pdrv->pi2c_client->addr = (pdata->pdrv->slave_addr>>1); 
        }
...
//vivo lishuo modify for otp remake 20210823 [B210820-3030] end
	return ret;
}
 ...
 * find_EEPROM_by_idx
 ******************************************************************************/
void *find_OTP_by_idx(unsigned char idx)
{
	void *pinst=NULL;

	if (idx>=MAX_EEPROM_NUMBER)
		return pinst;
	pinst = (void*)&ginst_drv[idx];

	return pinst;

}
EXPORT_SYMBOL(find_OTP_by_idx);
