...
/**
 * kbase_mem_pool_trylock - Lock a memory pool
 * @pool: Memory pool to lock
 */
static inline int kbase_mem_pool_trylock(struct kbase_mem_pool *pool)
{
	return spin_trylock(&pool->pool_lock);
}

/**
 * kbase_mem_init_max_slab_limit - init max slab limit
 */
static unsigned long max_slab_limit;
#define KBASE_MEM_SLAB_3GB (3*1024*1024)
#define KBASE_MEM_SLAB_5GB (5*1024*1024)
static inline void kbase_mem_init_max_slab_limit(struct kbase_device *kbdev)
{
	if ((totalram_pages << (PAGE_SHIFT - 10)) >= KBASE_MEM_SLAB_5GB) {
		/*for memtotal > 5GB, slab memory over 1.5G will force use lock*/
		max_slab_limit = 1500*1024;
	} else if ((totalram_pages << (PAGE_SHIFT - 10)) >= KBASE_MEM_SLAB_3GB) {
		/*for memtotal > 3GB && memtotal < 5GB, slab memory over 900MB will force use lock*/
		max_slab_limit = 900*1024;
	} else {
		/*for memtotal < 3GB, slab memory over 600MB will force use lock*/
		max_slab_limit = 600*1024;
	}

	dev_info(kbdev->dev, "%s:%d max_slab_limit:%lu\n", __FILE__, __LINE__, max_slab_limit);
}
