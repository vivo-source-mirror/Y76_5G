		...
		struct shrink_control *sc)
{
	...
	unsigned long slab_mem = 0;

	slab_mem = global_node_page_state(NR_SLAB_RECLAIMABLE) +
	...
	if (unlikely((slab_mem << (PAGE_SHIFT - 10)) > max_slab_limit))
		mutex_lock(&kctx->jit_evict_lock);
	else if (!mutex_trylock(&kctx->jit_evict_lock)) {
		// MTK add to prevent false alarm
		lockdep_on();
		return 0;
	}
