...
#ifndef CONFIG_MTK_TYPEC_WATER_DETEC
	struct work_struct usbid_detect_work;
	bool usbid_present;
	bool usbid_present_first_detect;
	struct delayed_work usbid_boot_detect;
#endif
#endif
};
...
static int mt6360_enable_usbid_polling(struct mt6360_chip *chip, bool en)
{
	...
	if (en) {
		...
		if (chip->chgdev && chip->chgdev->ops && chip->chgdev->ops->set_usbid_src_ton) {
			ret = chip->chgdev->ops->set_usbid_src_ton(chip->chgdev, 100000);
		}
#else
...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
		if (chip->chgdev && chip->chgdev->ops && chip->chgdev->ops->set_usbid_rup) {
			ret = chip->chgdev->ops->set_usbid_rup(chip->chgdev, 75000);
		}
#else
...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
	if (chip->chgdev && chip->chgdev->ops && chip->chgdev->ops->enable_usbid) {
		ret = chip->chgdev->ops->enable_usbid(chip->chgdev, en);
	}
#else
...
extern int vivo_get_usbid_present(struct charger_device *charger_dev);
extern int vivo_usbid_setting_first(struct charger_device *charger_dev);
//extern int vivo_usbid_setting_second(struct charger_device *charger_dev);
//extern int vivo_usbid_open_cc(struct charger_device *charger_dev);
extern void vote_typec_drp(const char *voter, bool drp);

static void vivo_mt6360_enable_usbid_irq(struct mt6360_chip *chip, bool en);

static void usbid_boot_detect_work(struct work_struct *work)
{
	int ret = 0;
	struct mt6360_chip *chip = container_of(work, struct mt6360_chip, usbid_boot_detect.work);

	if (!chip && !chip->chgdev) {
		pr_err("%s NULL, return\n", __func__);
		return;
	}

	ret = vivo_get_usbid_present(chip->chgdev);
	if (ret == 0) {
		pr_err("%s detect is 0\n", __func__);
		chip->usbid_present_first_detect = false;
		chip->usbid_present = false;
		vivo_usbid_setting_first(chip->chgdev);
		vote_typec_drp(USBID_TCPC_VOTER, false);
	} else {
		chip->usbid_present_first_detect = true;
		chip->usbid_present = true;
		vote_typec_drp(USBID_TCPC_VOTER, true);
	}
	pr_err("%s usbid is present:%d, usbid_present_first_detect: %d \n", __func__, chip->usbid_present, chip->usbid_present_first_detect);
}

static void vivo_usbid_detect_work(struct work_struct *work)
{
	int ret = 0;
	struct mt6360_chip *chip = container_of(work, struct mt6360_chip,
			usbid_detect_work);
...
static irqreturn_t mt6360_pmu_usbid_evt_handler(int irq, void *data)
{
	...
	cancel_work_sync(&chip->usbid_detect_work);
	schedule_work(&chip->usbid_detect_work);
...
#if defined(CONFIG_WD_SBU_POLLING) || defined(CONFIG_VIVO_USBID_ENABLE)
...
static void vivo_mt6360_enable_usbid_irq(struct mt6360_chip *chip, bool en)
{
	...
	pr_debug("%s", __func__);
	irq_desc = &mt6360_pmu_tcpc_irq_desc[0];
	if (en)
		enable_irq(irq_desc->irq);
	else
		disable_irq(irq_desc->irq);
...
#ifdef CONFIG_VIVO_USBID_ENABLE
#ifndef CONFIG_MTK_TYPEC_WATER_DETECT
	vivo_mt6360_enable_usbid_irq(chip, true);
#endif
#endif
	return 0;
...
static inline int mt6360_get_usbid_adc(struct tcpc_device *tcpc, int *usbid)
{
	...
	struct mt6360_chip *chip = tcpc_get_dev_data(tcpc);
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
	if (chip->chgdev && chip->chgdev->ops && chip->chgdev->ops->get_adc) {
		ret = chip->chgdev->ops->get_adc(chip->chgdev, ADC_CHANNEL_USBID, usbid, usbid);
	}
#else
...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
	if (chip->chgdev && chip->chgdev->ops && chip->chgdev->ops->set_usbid_rup) {
		ret = chip->chgdev->ops->set_usbid_rup(chip->chgdev, 1000);
	}
#else
...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
	if (chip->chgdev && chip->chgdev->ops && chip->chgdev->ops->set_usbid_rup) {
		chip->chgdev->ops->set_usbid_rup(chip->chgdev, 500000);
	}
#else
...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
	if (chip->chgdev && chip->chgdev->ops && chip->chgdev->ops->enable_usbid) {
		ret = chip->chgdev->ops->enable_usbid(chip->chgdev, false);
	}
#else
...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
	if (chip->chgdev && chip->chgdev->ops && chip->chgdev->ops->enable_usbid_floating) {
		ret = chip->chgdev->ops->enable_usbid_floating(chip->chgdev, false);
	}
#else
...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
	if (chip->chgdev && chip->chgdev->ops && chip->chgdev->ops->enable_usbid_floating) {
		ret = chip->chgdev->ops->enable_usbid_floating(chip->chgdev, true);
	}
#else
...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
	if (chip->chgdev && chip->chgdev->ops && chip->chgdev->ops->set_usbid_src_ton) {
		ret = chip->chgdev->ops->set_usbid_src_ton(chip->chgdev, 0);
	}
#else
...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
	if (chip->chgdev && chip->chgdev->ops && chip->chgdev->ops->set_usbid_rup) {
		ret = chip->chgdev->ops->set_usbid_rup(chip->chgdev, 500000);
	}
#else
...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
	if (chip->chgdev && chip->chgdev->ops && chip->chgdev->ops->enable_usbid) {
		ret = chip->chgdev->ops->enable_usbid(chip->chgdev, true);
	}
#else
...
err:
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
	if (chip->chgdev && chip->chgdev->ops) {
		if (chip->chgdev->ops->enable_usbid_floating)
			chip->chgdev->ops->enable_usbid_floating(chip->chgdev, true);
		if (chip->chgdev->ops->enable_usbid)
			chip->chgdev->ops->enable_usbid(chip->chgdev, false);
	}
#else
	charger_dev_enable_usbid_floating(chip->chgdev, true);
...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
			if (chip->chgdev && chip->chgdev->ops &&
					chip->chgdev->ops->get_ctd_dischg_status) {
				ret = chip->chgdev->ops->get_ctd_dischg_status(chip->chgdev, &status);
			}
#else
...
static int mt6360_tcpcdev_init(struct mt6360_chip *chip, struct device *dev)
{
	...
	if (IS_ERR(chip->tcpc) || !chip->tcpc)
		return -EINVAL;
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
	chip->tcpc->typec_role = desc->role_def + 1;
#endif
			    ...
			    const struct i2c_device_id *id)
{
	...
	INIT_WORK(&chip->usbid_detect_work, vivo_usbid_detect_work);
	INIT_DELAYED_WORK(&(chip->usbid_boot_detect), usbid_boot_detect_work);
	schedule_delayed_work(&(chip->usbid_boot_detect), msecs_to_jiffies(7000));
#endif
#endif
