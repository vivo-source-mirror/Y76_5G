...
int fulldp_mode = 1; 
extern int datadump_mode ;
static int dload_set(const char *val, const struct kernel_param *kp);
module_param_call(download_mode, dload_set, param_get_int, &download_mode, 0644);
static int dpmode_set(const char *val, const struct kernel_param *kp);
module_param_call(fulldp_mode, dpmode_set, param_get_int, &fulldp_mode, 0644);
static int param_get_datadump_mode(char *buffer,const struct kernel_param *kp);
module_param_call(datadump_mode, NULL, param_get_datadump_mode, NULL, 0444);
static int dload_set(const char *val, const struct kernel_param *kp)
{
	int ret;
...
static int dpmode_set(const char *val, const struct kernel_param *kp)
{
	...
	/* If fulldp_mode is not zero or one, ignore. */
	if (fulldp_mode >> 1) {
		fulldp_mode = old_val;
		return -EINVAL;
	}
	//set_dpmode(fulldp_mode);
...
static int param_get_datadump_mode(char *buffer,const struct kernel_param *kp)
{
	...
	if (datadump_mode == 0) 
		result = sprintf(buffer, "%x", 0);
	else
		result = sprintf(buffer, "%x", 1);
		...
		struct pt_regs *regs, const char *msg, ...)
{
	...
	if (mrdump_cblock && download_mode) {
		crash_record = &mrdump_cblock->crash_record;
