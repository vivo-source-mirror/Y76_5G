...
static ssize_t headset_key_store(struct device_driver *ddri, const char *buf, size_t count)
{

	if (sscanf(buf, "%d %d %d %d", &accdet_dts.four_key.mid,
				&accdet_dts.four_key.collect,
				&accdet_dts.four_key.up, &accdet_dts.four_key.down) == 4) {

		pr_info("%s() accdet key thresh: mid=%dmv,collect=%dmv,up=%dmv,down=%dmv\n", __func__,
				accdet_dts.four_key.mid, accdet_dts.four_key.collect,
				accdet_dts.four_key.up, accdet_dts.four_key.down);
	} else {
		pr_info("%s() change key-threshold err\n", __func__);
	}

	return count;
}

static ssize_t headset_key_show(struct device_driver *ddri, char *buf)
{
	int ret = 0;
	...
	ret = sprintf(buf, "accdet key thresh: mid=%dmv,collect=%dmv,up=%dmv,down=%dmv\n",
			accdet_dts.four_key.mid, accdet_dts.four_key.collect,
			accdet_dts.four_key.up, accdet_dts.four_key.down);
	pr_info("%s() ret=%d, buf_len=%d\n", __func__, ret, strlen(buf));
...
static DRIVER_ATTR_RW(headset_key);
...
cur_AB = pmic_read(PMIC_ACCDET_MEM_IN_ADDR) >> ACCDET_STATE_MEM_IN_OFFSET;
	...
	case MIC_BIAS:
		if (cur_AB == ACCDET_STATE_AB_00) {
			mutex_lock(&accdet_eint_irq_sync_mutex);
			if (eint_accdet_sync_flag && (!(gpio_get_value(gpiopin)))) {
				pr_info("%s, hook event of headset\n", __func__);
				s_button_status = 1;
				accdet_status = HOOK_SWITCH;
...
#endif
{
	...
	if (eint_accdet_sync_flag && (cable_type == NO_DEVICE)) {
		//clear_accdet_int();
...
static void accdet_init_once(void)
{
	...
	dump_register();
#endif
	//gaoansheng delay irq request here
	ret = request_irq(accdet_irq, ex_eint_handler, IRQF_TRIGGER_NONE, "accdet-eint", NULL);
	if (ret) {
	...
	ret = request_irq(accdet_r_irq, ex_r_eint_handler, IRQF_TRIGGER_NONE, "accdet-eint-r", NULL);
	if (ret) {
		pr_notice("accdet %s request_irq fail, ret:%d.\n", __func__, ret);
	}
