...
static atomic_t selftest_status;
static struct completion selftest_done;
...
static const struct mtk_nanohub_cmd mtk_nanohub_cmds[] = {
	...
	MTK_NANOHUB_CMD(SENSOR_HUB_RAW_DATA,
		mtk_nanohub_common_cmd),
	/*add by vsen team start*/
	MTK_NANOHUB_CMD(SENSOR_HUB_VSEN_CMD,
		mtk_nanohub_common_cmd),
	/*add by vsen team end*/
};
...
static void mtk_nanohub_get_sensor_info(void)
{
	...
	for (i = 0; i < k; ++i) {
		...
		support_sensors[j].version = sensor_state[i].version;
...
static void mtk_nanohub_init_sensor_info(void)
{
	...
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
	strlcpy(p->name, "angle_judge", sizeof(p->name));
	strlcpy(p->vendor, "vivo", sizeof(p->vendor));

	p = &sensor_state[COMM_SENSOR_TYPE_ANGLE_DIRECTION];
	p->sensorType = COMM_SENSOR_TYPE_ANGLE_DIRECTION;
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
	strlcpy(p->name, "angle_direction", sizeof(p->name));
	strlcpy(p->vendor, "vivo", sizeof(p->vendor));

	p = &sensor_state[COMM_SENSOR_TYPE_RAISEUP_DETECT];
	p->sensorType = COMM_SENSOR_TYPE_RAISEUP_DETECT;
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
	strlcpy(p->name, "raiseup_detect", sizeof(p->name));
	strlcpy(p->vendor, "vivo", sizeof(p->vendor));

	p = &sensor_state[COMM_SENSOR_TYPE_PUTDOWN_DETECT];
	p->sensorType = COMM_SENSOR_TYPE_PUTDOWN_DETECT;
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
	strlcpy(p->name, "putdown_detect", sizeof(p->name));
	strlcpy(p->vendor, "vivo", sizeof(p->vendor));

	p = &sensor_state[COMM_SENSOR_TYPE_VIVOMOTION_DETECT];
	p->sensorType = COMM_SENSOR_TYPE_VIVOMOTION_DETECT;
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
	strlcpy(p->name, "vivomotion_detect", sizeof(p->name));
	strlcpy(p->vendor, "vivo", sizeof(p->vendor));

	p = &sensor_state[COMM_SENSOR_TYPE_AMD];
	p->sensorType = COMM_SENSOR_TYPE_AMD;
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
	strlcpy(p->name, "amd", sizeof(p->name));
	strlcpy(p->vendor, "vivo", sizeof(p->vendor));

	p = &sensor_state[COMM_SENSOR_TYPE_SMARTPROX_DETECT];
	p->sensorType = COMM_SENSOR_TYPE_SMARTPROX_DETECT;
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
	strlcpy(p->name, "smartprox_detect", sizeof(p->name));
	strlcpy(p->vendor, "vivo", sizeof(p->vendor));

	p = &sensor_state[COMM_SENSOR_TYPE_WINDOW_ORIENTATION];
	p->sensorType = COMM_SENSOR_TYPE_WINDOW_ORIENTATION;
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
	strlcpy(p->name, "window_orientation", sizeof(p->name));
	strlcpy(p->vendor, "vivo", sizeof(p->vendor));

	p = &sensor_state[COMM_SENSOR_TYPE_AMBIENT_LIGHT_SCENE];
	p->sensorType = COMM_SENSOR_TYPE_AMBIENT_LIGHT_SCENE;
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
...
static int mtk_nanohub_report_data(struct data_unit_t *data_t)
{
	...
	/* add by vsen team : printk ps event time begin*/
	int64_t now_enter_timestamp = 0;
	now_enter_timestamp = ktime_get_boot_ns();
	if (data_t->sensor_type ==  ID_PROXIMITY) {
		pr_err("recv prox distance:%d(%d) ap_time=%lld evt_time=%lld\n", data_t->proximity_t.oneshot, data_t->proximity_t.steps, now_enter_timestamp, data_t->time_stamp);
	}
	/* add by vsen team : printk ps event time end*/

	if (data_t->sensor_type ==  ID_VIVOMOTION_DETECT) {
		pr_err("recv vivomotion distance:%d ap_time=%lld evt_time=%lld\n", (int)data_t->data[0], now_enter_timestamp, data_t->time_stamp);
	}
...
int mtk_nanohub_cfg_to_hub(uint8_t sensor_id, uint8_t *data, uint8_t count)
{
	...
	int ret = 0;
	pr_err("%s sensor_id = [%d], data[0] = %d, data[1] = %d, data[2] = %d\n", __func__, sensor_id, data[0], data[1], data[2]);
...
int mtk_nanohub_calibration_to_hub(uint8_t sensor_id)
{
	...
	struct ConfigCmd cmd;
	int ret = 0;
	pr_err("mtk_nanohub_calibration_to_hub_id= %d\n", sensor_id);
...
static void mtk_nanohub_restoring_config(void)
{
	...
	if (data) {
		spin_lock(&config_data_lock);
		memcpy(data, device->sarSecondary_config_data, length);
		spin_unlock(&config_data_lock);
...
void mtk_nanohub_power_up_loop(void *data)
{
	...
	pr_err("scp_sensor_fifo =%p, wp =%d, rp =%d, size =%d\n",
...
int vivo_nanohub_selftest(int sensor_type)
{
	...
	if (sensor_type <= 0)
		return 0;
	pr_err("%s [%d] time in:%llu\n", __func__, sensor_type, ktime_get_boot_ns());

	atomic_set(&selftest_status, 0);

	mtk_nanohub_selftest_to_hub(type_to_id(sensor_type));

	ret = wait_for_completion_timeout(&selftest_done, msecs_to_jiffies(3000));
	pr_err("%s ret = %d time out:%llu\n", __func__, ret, ktime_get_boot_ns());
	if (!ret)
		return -1;
...
static int mtk_nanohub_report_to_manager(struct data_unit_t *data)
{
	...
	if (data->flush_action == DATA_ACTION) {
		...
		case ID_STEP_COUNTER:
			...
			pr_err("mtk_nanohub_report_to_manager[step_counter]: %d, timestamp:%lld\n",
		...
		event.word[4] = data->data[4];
		event.word[5] = data->data[5];
		pr_err("mtk_nanohub_report_to_manager_else: action:%d event.word[0]=%d event.word[1]=%d event.word[2]=%d event.word[3]=%d,event.word[4]=%d,event.word[5]=%d\n",
				event.action, event.word[0], event.word[1], event.word[2], event.word[3], event.word[4], event.word[5]);
	}
	/*
	...
	if (data->flush_action == DATA_ACTION) {
		if (data->sensor_type == ID_PROXIMITY ||
			...
			data->sensor_type == ID_VIVOMOTION_DETECT ||
			data->sensor_type == ID_SIGNIFICANT_MOTION ||
			data->sensor_type == ID_RAISEUP_DETECT ||
			data->sensor_type == ID_AMD ||
			data->sensor_type == ID_SMARTPROX_DETECT ||
			data->sensor_type == ID_AMBIENT_LIGHT_SCENE ||
			data->sensor_type == ID_SAR ||
			data->sensor_type == ID_SAR_SECONDARY ||
	...
	/*pr_err("mtk_nanohub_report_to_manager type:%d x=%d y=%d z=%d"
		"event.word[3]=%d,event.word[4]=%d,event.word[5]=%d,action=%d, timestamp:%lld\n",
		event.sensor_type, event.word[0], event.word[1], event.word[2],
		event.word[3], event.word[4], event.word[5], event.action, event.timestamp);*/
			...
			void *unused)
{
	...
	case PM_POST_SUSPEND:
		pr_err("resume ap boottime=%lld\n", ktime_get_boot_ns());
		WRITE_ONCE(rtc_compensation_suspend, false);
		mtk_nanohub_send_timestamp_to_hub();
		return NOTIFY_DONE;
	case PM_SUSPEND_PREPARE:
		pr_err("suspend ap boottime=%lld\n", ktime_get_boot_ns());
