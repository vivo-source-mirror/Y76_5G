...
#define  USB_OTG_VOTER "USB_OTG_VOTER"
...
//	vivo_set_typec_power_role(TYPEC_ROLE_SNK);
	vote_typec_drp(USB_OTG_VOTER, false);
...
void mt_usbaudio_connect(void)
{
	if (typec_hw_det) {
		...
		pr_info("%s, host_mode_enabled:%d \n", __func__, host_mode_enabled);
	}
}
...
static ssize_t otg_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	pr_err(" %s, %d, typec_hw_det:%d\n", __func__, __LINE__, typec_hw_det);
...
//			vivo_set_typec_power_role(TYPEC_ROLE_DRP);
			vote_typec_drp(USB_OTG_VOTER, true);
...
//			vivo_set_typec_power_role(TYPEC_ROLE_SNK);
		...
		issue_host_work(CONNECTION_OPS_CHECK, 0, false);
	}
}
...
static int mt_usb_probe(struct platform_device *pdev)
{
	...
	typec_hw_det = of_property_read_bool(np, "vivo,typec-hw-det");
	dev_info(&pdev->dev, "typec_hw_det:%d\n", typec_hw_det);
