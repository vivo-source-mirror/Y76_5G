...
// vivo add start
// add tee work affinity control sysfs
#ifdef TEE_WORK_AFFINITY_SYSFS
#define TEE_WORK_AFFINITY_BIG_CORE_MASK 0xc0
struct kobject tee_work_affinity_kobj;
static int tee_work_affinity_big_core;
static atomic_t big_core_request_cnt;

static ssize_t tee_work_affinity_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	long tee_affinity = 0;

	kstrtol(buf, 16, &tee_affinity);
	if (!tee_affinity) {
	    goto err_input;
	}

	printk(KERN_INFO "%s: %d: tee dest_mask = 0x%lx, cur_mask = 0x%lx", __func__, __LINE__, tee_affinity, l_ctx.default_affinity_mask);
	tee_affinity &= l_ctx.default_affinity_mask;
	if (!tee_affinity) {
		mc_dev_devel("Can't have an empty affinity.");
		mc_dev_devel("Restoring the default mask");
...
err_input:
    printk(KERN_INFO "%s: %d: Invalid input", __func__, __LINE__);
    return -EINVAL;
}

static ssize_t tee_work_affinity_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    int tee_work_affinity = get_tee_affinity();
    return snprintf(buf, 4, "%x\n", tee_work_affinity);
}

static ssize_t tee_work_affinity_big_core_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    int big_core, ret;

    ret = kstrtoint(buf, 10, &big_core);
    if (ret) {
        goto err_input;
    }

    tee_work_affinity_big_core = big_core;

    printk(KERN_INFO "%s: %d: tee work %s", __func__, __LINE__, big_core ? "big_affinity" : "default_affinity");

    if (big_core) {
		atomic_inc(&big_core_request_cnt);
		atomic_set(&l_ctx.tee_affinity, TEE_WORK_AFFINITY_BIG_CORE_MASK);
    } else {
        /* decrease state in case of several overlapping requests */
        if (atomic_dec_if_positive(&big_core_request_cnt) <= 0)
...
static ssize_t tee_work_affinity_big_core_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return snprintf(buf, 4, "%d\n", tee_work_affinity_big_core);
}

static struct kobj_attribute tee_work_affinity_attribute =
    __ATTR(affinity, 0644, tee_work_affinity_show, tee_work_affinity_store);

static struct kobj_attribute tee_work_affinity_big_core_attribute =
    __ATTR(big_core, 0644, tee_work_affinity_big_core_show, tee_work_affinity_big_core_store);

static struct attribute *tee_work_own_sys_attrs[] = {
    &tee_work_affinity_attribute.attr,
    &tee_work_affinity_big_core_attribute.attr,
    NULL,
};


static ssize_t tee_work_affinity_object_show(struct kobject *k, struct attribute *attr, char *buf)
{
...
static ssize_t tee_work_affinity_object_store(struct kobject *k, struct attribute *attr, const char *buf, size_t count)
{
    struct kobj_attribute *kobj_attr;
    int ret = -EIO;
...
static void tee_work_affinity_object_release(struct kobject *kobj)
{
    /* nothing to do temply */
    return;
}

static const struct sysfs_ops tee_work_affinity_object_sysfs_ops = {
    .show = tee_work_affinity_object_show,
    .store = tee_work_affinity_object_store,
};
static struct kobj_type tee_work_affinity_object_type = {
    .sysfs_ops  = &tee_work_affinity_object_sysfs_ops,
    .release    = tee_work_affinity_object_release,
    .default_attrs = tee_work_own_sys_attrs,
};
#endif
// vivo add end
...
int nq_init(void)
{
	...
	// add tee work affinity control sysfs
	#ifdef TEE_WORK_AFFINITY_SYSFS
	atomic_set(&big_core_request_cnt, 0);
	if (kobject_init_and_add(&tee_work_affinity_kobj, &tee_work_affinity_object_type, NULL, "tee_work")) {
		printk(KERN_INFO "%s: %d: Create sys/tee_work_affinity failed",__func__, __LINE__);
	} else {
		printk(KERN_INFO "%s: %d: Create sys/tee_work_affinity success",__func__, __LINE__);
	}
	#endif
	// vivo add end
	// add tee work affinity control sysfs
