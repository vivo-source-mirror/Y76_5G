...
//		char puSendCmd1[2] = {(char)(0x02), (char)(0x01)};//Power down
	...
	LOG_INF("initAF + vivo add by 2021/11/01\n");//zjj add for AF due to reboot
		g_pstAF_I2Cclient->addr = AF_I2C_SLAVE_ADDR;
	LOG_INF("initAF - vivo add by 2021/11/01\n");
		g_pstAF_I2Cclient->addr = g_pstAF_I2Cclient->addr >> 1;
	LOG_INF("g_pstAF_I2Cclient->addr = 0x%x add by 2021/11/01\n",g_pstAF_I2Cclient->addr);
	...
	if (*g_pAF_Opened == 1) {
		...
		LOG_INF("i4RetValue(2) vivo  add by 2021/11/01\n");
		if (i4RetValue < 0)
		...
		LOG_INF("i4RetValue(3) vivo  add by 2021/11/01\n");
		if (i4RetValue < 0)
		...
		LOG_INF("i4RetValue(4) vivo add by 2021/11/01\n");
		if (i4RetValue < 0)
		...
		LOG_INF("i4RetValue(5) vivo add by 2021/11/01\n");
		if (i4RetValue < 0)
		...
		LOG_INF("i4RetValue(6) vivo add by 2021/11/01\n");
		if (i4RetValue < 0)
		...
		LOG_INF("spin_lock vivo add by 2021/11/01\n");
		*g_pAF_Opened = 2;
		LOG_INF("g_pAF_Opened = 2 vivo  add by 2021/11/01\n");
		spin_unlock(g_pAF_SpinLock);
		LOG_INF("spin_unlock vivo  add by 2021/11/01\n");
	}
...
{
	LOG_INF("vivo0 + g_pstAF_I2Cclient=%p,&g_pstAF_I2Cclient=%p\n",g_pstAF_I2Cclient,&g_pstAF_I2Cclient);
