...
enum ACDK_SENSOR_FEATURE_ENUM {
	...
	SENSOR_FEATURE_GET_RAW_INFO, //vivo rcs add raw_info 8.13
	...
	SENSOR_FEATURE_GET_CUSTOM_INFO,		/*vivo hope add */
	...
	SENSOR_FEATURE_GET_CTID, //add by zhanghainan
	...
	SENSOR_FEATURE_GET_HS_TRAIL,/*zjj add for video caton (B210703-2473) 2021/7/5*/
	SENSOR_FEATURE_MAX
};
...
struct SENSOR_RAWINFO_STRUCT {
	MUINT16 raw_weight; 
	MUINT16 raw_height;
	MUINT16 raw_dataBit;
	MUINT16 raw_colorFilterValue;
	MUINT16 raw_blackLevel;
	MFLOAT  raw_viewAngle;
	MUINT16 raw_bitWidth;
	MUINT16 raw_maxSensorGain;
};
...
enum IMGSENSOR_COLOR_FILTER_TYPE {
	BAYER_GRBG,
	BAYER_BGGR,
	BAYER_GBRG,
	BAYER_RGGB,
...
#define OTP_CLOUD_FLAG_ADDRSHIFT 0x13
typedef enum {   
	OTP_TYPE_NORMAL = 0,
    OTP_TYPE_EMPTY,
    OTP_TYPE_CLOUD_NONE_EERPOM,
    OTP_TYPE_CLOUD_WITH_EERPOM,
    OTP_TYPE_VITRUAL,
    OTP_TYPE_MAX
} otp_type_code_t;
...
#pragma pack(push) 
#pragma pack(1)
struct IMGSENSOR_SENSOR_TABLEINFO{
    //otp partition table
    const MUINT16 ModuleInfo_addr;
        const MUINT16 PLATFORM_id_addr;
        const MUINT16 LENS_id_addr;
        const MUINT16 VCM_id_addr;
        const MUINT16 DRIVERIC_id_addr;
        const MUINT16 CalibrationVersion_addr;  
        const MUINT16 Material_inf_addr;
    const MUINT16 ModuleInfo_checksum_addr;
    const MUINT16 Fuse_id_addr;
    const MUINT16 Fuse_id_checksum_addr;
    const MUINT16 SN_addr;
    const MUINT16 SN_checksum_addr;
    const MUINT16 Af_addr;
    const MUINT16 Af_checksum_addr;
    const MUINT16 Awb_addr;
    const MUINT16 Awb_checksum_addr;
    const MUINT16 Lsc_addr;
    const MUINT16 Lsc_checksum_addr;
    const MUINT16 PDAF_proc1_addr;
    const MUINT16 PDAF_proc1_checksum_addr;
    const MUINT16 PDAF_proc2_addr;
    const MUINT16 PDAF_proc2_checksum_addr;
    const MUINT16 Cross_talk_addr;
    const MUINT16 Cross_talk_checksum_addr;
    const MUINT16 XTC_cal_addr;
    const MUINT16 XTC_cal_checksum_addr;
    const MUINT16 SensorXTC_cal_addr;
    const MUINT16 SensorXTC_cal_checksum_addr;	 
    const MUINT16 PDXTC_addr;
    const MUINT16 PDXTC_checksum_addr;
    const MUINT16 SW_GGC_addr;
    const MUINT16 SW_GGC_checksum_addr;
    const MUINT16 HW_GGC_addr;
    const MUINT16 HW_GGC_addr_checksum_addr;

    const MUINT16 OIS_addr;
    const MUINT16 OIS_checksum_addr;
    const MUINT16 OIS_shift_calibration_addr;
    const MUINT16 OIS_shift_calibration_checksum_addr;

    MUINT8 *otpData;  //otpdata pointer,only use for hal.Init in hal CamCalHelper::OTP_Initbuff
    
//OTP other info
    const MUINT16 OTP_addr_offset;
    const MUINT16 OTP_size;
    const MUINT8  OTP_slave_addr;
    MUINT8 OTP_type;
    MUINT8  Fuse_id[32];
    const MUINT8  Position_type[16];
    const MUINT8  Module_type[16];
    MUINT8  Material_inf[32];
    MUINT8  SN_inf[32];

    const MUINT8  PLATFORM_id;
	const MUINT8  LENS_id;
    const MUINT8  VCM_id;
    const MUINT8  DRIVERIC_id;	
    const MUINT8  CalibrationVersion[4];
    
    const MUINT8  MtkLscType;
    const MUINT8  PixId;
	const MUINT8  TableRotation; 
    
    MUINT8  Af_info[6]; //afcode function,FIX ME
};
...
struct SENSOR_FUNCTION_STRUCT {
	...
	MUINT32 (*SensorHop)(int simid, MUINT8 rat, MUINT32 arfcn);
	MUINT32 (*SensorDoHop)(int flag, void *psensor);
	// Modify for CCM Hopping End
	MUINT32 (*SensorGetOtpInfo)(struct IMGSENSOR_SENSOR_TABLEINFO **p); //vivo lishuo add for otp remake 20210823 [B210820-3030]
...
// Modify for CCM Hopping Start
enum MODEM_OPERATION_RAT {
	MODEM_OPERATION_RAT_NULL = 0,
	MODEM_OPERATION_RAT_GSM = 1,
	MODEM_OPERATION_RAT_UMTS = 2,
	MODEM_OPERATION_RAT_WIFI = 3,
	MODEM_OPERATION_RAT_LTE = 4,	/*LTE and 5G sharing*/
};

#define MODEM_SIM_IDLE  0
#define MODEM_SIM_CONN  1
#define MODEM_SIM_NULL  0xff

#define L4C_MAX_ARFCN_SUPPORTED 8
#define MODEM_SUPPORTED_SIM_MAXNUM 4
struct rmmi_camera_arfcn_info_struct {
  MUINT8 simid;
  MUINT8 rat;
  MUINT8 num_of_arfcn;
  MUINT32 arfcn[L4C_MAX_ARFCN_SUPPORTED];
  MUINT16 band[L4C_MAX_ARFCN_SUPPORTED];
  MUINT8 is_connected;
  MUINT8 is_main_sim;
} __packed;

struct wifi2camera_info_struct {
	MUINT8         num_of_wifi_CH;
	MUINT32       wifi_CH[2];
	MUINT8          is_wifiConnected;
	MUINT8          is_wifiOn;
};
