 ...
 * Filename:
 * ---------
 *	 s5kjn1sq03v1pd2164mipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#define PFX "MAIN[38E2]_camera_sensor"
...
#include "s5kjn1sq03v1pd2164mipiraw_freq.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJN1SQ03V1PD2164_SENSOR_ID,
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info =  //4080X3072
{
	...
	.i4BlockNumY = 382,
	.iMirrorFlip = 3,
	.i4Crop = { {0,0}, {0,0}, {0, 382}, {0, 0}, {0, 0}, {0,0}, {120, 450}, {0, 0}, {0, 382}, {120, 450}},
};
#endif
/*hope add otp check start*/
#if 1
static int vivo_otp_read_when_power_on;
extern int MAIN_38E2_otp_read(void);
extern otp_error_code_t S5KJN1SQ03V1PD2164_OTP_ERROR_CODE;
MUINT32  af_calib_inf_main_s5kjn1sq03v1pd2164[6];
#endif
MUINT32  sn_inf_main_s5kjn1sq03v1pd2164[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_s5kjn1sq03v1pd2164[4];
MUINT32  crosstalk_data_V1PD2164[8006];  /*0 flag   1-836 data*/
...
#endif
extern bool vivo_read_eeprom_v1(kal_uint16 addr,  BYTE *data);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	kal_uint8 retry = 2;
 	int offset = 0xF;
	unsigned char material_inf_main_s5kjn1sq03v1[4];
		...
		do {
			*sensor_id = (read_cmos_sensor_16_16(0x0000) + 1);
			PK_DBG("read out sensor id 0x%x \n",*sensor_id);
				for(j = 0;j < 4;j++){
					vivo_read_eeprom_v1(offset,&material_inf_main_s5kjn1sq03v1[j]);	
					offset++;
				}			
				if((int)material_inf_main_s5kjn1sq03v1[3] == 0x53) {
			if ( *sensor_id == imgsensor_info.sensor_id) {
				...
				vivo_otp_read_when_power_on = MAIN_38E2_otp_read();
				pr_debug("read eeprom ---vivo_otp_read_when_power_on = %d,MAIN_F8D1_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, S5KJN1SQ03V1PD2164_OTP_ERROR_CODE);
				/*vivo lxd add end*/
				return ERROR_NONE;
			}
			}
...
#ifdef HOPE_ADD_FOR_CAM_TEMPERATURE_NODE  //add for s5kjn1sq03v1pd2164 temperature 
    ...
    */
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = S5KJN1SQ03V1PD2164_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_s5kjn1sq03v1pd2164, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main_s5kjn1sq03v1pd2164, sizeof(MUINT32)*4); 
				memcpy( feature_data+12, af_calib_inf_main_s5kjn1sq03v1pd2164, sizeof(MUINT32)*6);
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_s5kjn1sq03v1pd2164[%d]= 0x%x\n", i, sn_inf_main_s5kjn1sq03v1pd2164[i]);
						}
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:/*get 4 cell data from eeprom*/
	{
		...
		if (type == FOUR_CELL_CAL_TYPE_XTALK_CAL) {
			...
			data[1] = 0x1F;
			for(i = 0; i< 8006 ; i++)
			data[2 + i] = crosstalk_data_V1PD2164[i];
...
UINT32 S5KJN1SQ03V1PD2164_MIPI_RAW_SensorInit(
