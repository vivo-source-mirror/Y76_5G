...
extern void *find_OTP_by_idx(unsigned char idx); //vivo lishuo add for otp remake 20210823 [B210820-3030] add	

static int imgsensor_analyseOTPtype(struct IMGSENSOR_SENSOR_INST *psensor_inst){
    struct EEPROM_DRV *pEEpInfo =  psensor_inst->pEEpInfo;
    struct IMGSENSOR_SENSOR_TABLEINFO *ptableInfo = psensor_inst->psensor_tableInfo;
    MINT32 idx = psensor_inst->sensor_idx;
	u8 *buff,length,i,otp_cloud_flag;
	u16 sum=0;
    
    pEEpInfo = find_OTP_by_idx(psensor_inst->sensor_idx);
    if(pEEpInfo == NULL || ptableInfo == NULL){
        PK_DBG("[imgsensor_analyseOTPtype]EEpInfo/tableInfo is null,sensor_idx:%x\n", idx);   
        return 0;
    }

	pEEpInfo->otp_MaxAddr = 0;
	pEEpInfo->slave_addr = 0;

    if(ptableInfo->OTP_type == OTP_TYPE_VITRUAL){ 
        return 1;
    }

    if (test_i2c_is_timeout(ptableInfo->OTP_slave_addr)) {  
        PK_DBG("[%s]eeprom i2c read 0x%02x failed\n",__func__,ptableInfo->OTP_slave_addr);
        ptableInfo->OTP_type = OTP_TYPE_EMPTY;        
        if(ptableInfo->Fuse_id[0]!='\0'){
            ptableInfo->OTP_type = OTP_TYPE_CLOUD_NONE_EERPOM; 
            return 1;
        }
    }
    else{
	    pEEpInfo->otp_MaxAddr = ptableInfo->OTP_size + ptableInfo->OTP_addr_offset;
        pEEpInfo->slave_addr = ptableInfo->OTP_slave_addr;
        otp_read(ptableInfo->OTP_addr_offset + OTP_CLOUD_FLAG_ADDRSHIFT, \
                1, &otp_cloud_flag, ptableInfo->OTP_slave_addr); 		
        if(otp_cloud_flag == 1){
            ptableInfo->OTP_type = OTP_TYPE_CLOUD_WITH_EERPOM;    
            return 1;  
        }     
        else{
            ptableInfo->OTP_type = OTP_TYPE_NORMAL;          
        }        
        //vivo lishuo add for AFcode,hardcore FIX ME ,begin
        if(ptableInfo->Af_checksum_addr > ptableInfo->Af_addr){
            length = ptableInfo->Af_checksum_addr - ptableInfo->Af_addr + 1;
            buff = kzalloc(length, GFP_KERNEL);
            if(buff != NULL){
                otp_read(ptableInfo->OTP_addr_offset + ptableInfo->Af_addr, \
                    length, buff, ptableInfo->OTP_slave_addr);
                if(buff[0] == 1){  //is vaild
                    for(i = 1; i< length - 1;i++){
                        sum+=buff[i];
                    }
                    sum = sum%255 + 1;
                    if(sum == buff[length - 1]){
                        ptableInfo->Af_info[0] = buff[7];
                        ptableInfo->Af_info[1] = buff[8];
                        ptableInfo->Af_info[2] = buff[9];
                        ptableInfo->Af_info[3] = buff[10];
                        ptableInfo->Af_info[4] = buff[11];
                        ptableInfo->Af_info[5] = buff[12];
                        kfree(buff);
                        PK_DBG("[%s]eeprom i2c read afcode success\n",__func__);
                        return 1;
                    }
                    else{
                        PK_DBG("[%s]eeprom i2c read afcode checksum failed\n",__func__);    
                    }
                }
                kfree(buff);
            }
        ...
        //vivo lishuo add for AFcode,hardcore FIX ME ,end
        return 0;   
   }
   return 0;
}
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
    ...
    //vivo zhouyikuan modify for vcamio pumped camera stuck with Logic optimization
	if (pimgsensor->imgsensor_oc_irq_enable != NULL)
		    pimgsensor->imgsensor_oc_irq_enable(
		    psensor->inst.sensor_idx, true);
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {
		...
		if(psensor->pfunc->SensorGetOtpInfo==NULL){
			printk("psensor%d->pfunc.SensorGetOtpInfo is NULL\n",psensor_inst->sensor_idx);	
		}
		else{
			printk("psensor%d->pfunc.SensorGetOtpInfo is not NULL\n",psensor_inst->sensor_idx);
			imgsensor_mutex_lock(&psensor->inst);
			psensor->pfunc->SensorGetOtpInfo(&psensor->inst.psensor_tableInfo);
			imgsensor_analyseOTPtype(&psensor->inst);   
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	case SENSOR_FEATURE_GET_AWB_REQ_BY_SCENARIO:
		{
			...
			*(pFeaturePara_64 + 1) = (uintptr_t)&Value;
			...
			ret = imgsensor_sensor_feature_control(psensor,
					...
					(unsigned int *)&FeatureParaLen);
#if defined CONFIG_MTK_CAM_PD2156  || defined CONFIG_MTK_CAM_PD2156F_EX
			if(pFeatureCtrl->FeatureId != SENSOR_FEATURE_GET_CUSTOM_INFO){ //vivo lishuo add for otp remake 20210823 [B210820-3030]
	...
	case SENSOR_FEATURE_AUTOTEST_CMD:
	{
		...
		*(pFeaturePara_64) = (uintptr_t)&Value0;
		*(pFeaturePara_64 + 1) = (uintptr_t)&Value1;
		...
		*(pFeaturePara_64) = Value0;
		*(pFeaturePara_64 + 1) = Value1;
    }
	break;
