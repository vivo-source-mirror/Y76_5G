...
#define VIVO_OTP_DATA_SIZE 0x1a2f /*sizeof(moduleinfo)+sizeof(awb)+sizeof(af)+sizeof(lsc)*/
...
static unsigned const int PD_Proc2_checksum_addr = 0x1a2e;
#ifdef AF_RANGE_GOT
...
otp_error_code_t OV48B2Q_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
unsigned  int ov48b2q_flag;
extern MUINT32  sn_inf_main_ov48b2q[13];  /*0 flag   1-12 data*/
extern MUINT32  material_inf_main_ov48b2q[4];
...
int ov48b2q_vivo_otp_read(void)
{
	int i = 0;
	...
	OV48B2Q_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	...
	if (vivo_otp_data[0x0A] != 0x9 ){
		OV48B2Q_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
		ov48b2q_flag = 0;
		LOG_INF("VCM ID Error!!!    ov48b2q_flag =%d\n", ov48b2q_flag);
	}else{
		ov48b2q_flag = 1;
		LOG_INF("ov48b2q_flag = %d\n", ov48b2q_flag);
	}
	...
	if (check_if_group_valid == 0x01) //all the data is valid
	{
		...
		{
			OV48B2Q_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("ModuleInfo_info error!!!    otp_error_code:%d\n",OV48B2Q_OTP_ERROR_CODE);
			return 0;
		} else if (((vivo_otp_data[0x02] != 0x03) ||(vivo_otp_data[0x08] != 0x42) ||(vivo_otp_data[0x07] != 0x48) ||(vivo_otp_data[0x06] != 0x56)|| ((vivo_otp_data[0x09] != 0x01) && (vivo_otp_data[0x09] != 0x01)) || (vivo_otp_data[0x0A] != 0x09) || (vivo_otp_data[0x0B] != 0x03)) &&(vivo_otp_data[0x0B] != 0x03))
		{
			OV48B2Q_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("Platform ID or Sensor or Lens or VCM ID or Driver_IC ID  Error!!!    otp_error_code:%d\n",OV48B2Q_OTP_ERROR_CODE);
			return 0;
		} else if ((vivo_otp_data[0x0A] != 0x09)) {
			OV48B2Q_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
        ...
        {
			OV48B2Q_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		sn_inf_main_ov48b2q[0] = 0x01;
		for(i = 0;i < 12;i++){
			sn_inf_main_ov48b2q[i +1] = (MUINT32)vivo_otp_data[i + SNInfo_addr+1];
			//LOG_INF("sn_inf_main_ov48b2q[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_main_ov48b2q[i +1],  i +SNInfo_addr+1, vivo_otp_data[i + SNInfo_addr+1]);
		}
		
		//////Material_addr_checksum
#if 1
		material_inf_main_ov48b2q[0] = vivo_otp_data[0x10];
		material_inf_main_ov48b2q[1] = vivo_otp_data[0x11];
		material_inf_main_ov48b2q[2] = vivo_otp_data[0x12];
		material_inf_main_ov48b2q[3] = vivo_otp_data[0x13];
#endif		
		//////AWB_checksum
		checksum = 0;
        ...
        if(vivo_otp_data[0x01] == 0x05){    
			...
			if (diff_inf_137 > 150 || diff_mac_137 > 150 || diff_inf_macro > 450 || diff_inf_macro < 150) {  /*AF code out of range*/
			    OV48B2Q_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
			    LOG_INF("QTech 2311137 AF out of range error!!!   otp_error_code:%d   VCM ID = 0x%x AF_inf = %d AF_mac = %d\n", OV48B2Q_OTP_ERROR_CODE, vivo_otp_data[0x0A], AF_inf, AF_mac);
			    return 0;
		    }
        }
		...
		OV48B2Q_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
