...
extern int imx355_otp_read(void);
int vivo_imx355_otp_read_when_power_on;
extern otp_error_code_t IMX355_OTP_ERROR_CODE;
MUINT32  sn_inf_sub_imx355[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_sub_imx355[4];  
...
#endif
static kal_uint16 imx355_table_write_cmos_sensor(
...
kal_uint16 addr_data_pair_init_imx355[] = {
	0x4348, 0x16,
	0x4350, 0x19,
	0x4408, 0x0A,
	0x440C, 0x0B,
...
/*3264x2448@30fps*/
kal_uint16 addr_data_pair_full_size_imx355[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x0E,
...
/*3264x1840@30fps*/
kal_uint16 addr_data_pair_crop_size_imx355[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x0E,
...
/*1632x1224@30fps*/
kal_uint16 addr_data_pair_bining_size_imx355[] = {
	0x0112, 0x0A,
	0x0113, 0x0A,
	0x0114, 0x03,
	0x0342, 0x07,
...
{
	imx355_table_write_cmos_sensor(addr_data_pair_init_imx355,
...
{
	imx355_table_write_cmos_sensor(addr_data_pair_full_size_imx355,
	sizeof(addr_data_pair_full_size_imx355) / sizeof(kal_uint16));
}

static void crop_size_setting(void)
{
	imx355_table_write_cmos_sensor(addr_data_pair_crop_size_imx355,
	sizeof(addr_data_pair_crop_size_imx355) / sizeof(kal_uint16));
}

static void binning_size_setting(void)
{
	imx355_table_write_cmos_sensor(addr_data_pair_bining_size_imx355,
	sizeof(addr_data_pair_bining_size_imx355) / sizeof(kal_uint16));
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	 ...
	 */
		...
		kal_uint8 timeout = 0;
		timeout = check_i2c_timeout(0x0a0a, imgsensor.i2c_write_id);
	    if (timeout) {
		...
		do {
			*sensor_id = return_type_id_from_otp();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				vivo_imx355_otp_read_when_power_on = imx355_otp_read();
				LOG_INF("cfx_add:end read eeprom ---vivo_imx355_otp_read_when_power_on = %d,IMX355_OTP_ERROR_CODE =%d\n", vivo_imx355_otp_read_when_power_on, IMX355_OTP_ERROR_CODE);
				return ERROR_NONE;
			}
			LOG_INF(
...
static kal_uint32 open(void)
{
	...
	LOG_1;
	LOG_INF("imx355 open begin..\n");
	KD_SENSOR_PROFILE_INIT();
	get_imgsensor_id(&sensor_id);
	if (imgsensor_info.sensor_id != sensor_id)
		return ERROR_SENSOR_CONNECT_FAIL;
	KD_SENSOR_PROFILE("imx355 get sensor id time");
	/* initail sequence write in  */
	global_setting();
	KD_SENSOR_PROFILE("imx355 set global setting time");
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	KD_SENSOR_PROFILE("imx355 set binning size setting time");
	return ERROR_NONE;
}
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_INF("imx355 preview capture..\n");
	spin_lock(&imgsensor_drv_lock);
	...
	KD_SENSOR_PROFILE_INIT();
	full_size_setting();
	KD_SENSOR_PROFILE("imx355 set full size setting time");
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	KD_SENSOR_PROFILE_INIT();
	crop_size_setting();
	KD_SENSOR_PROFILE("imx355 set crop size setting time");
...
static kal_uint32 imx355_awb_gain(struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
 ...
 */
	...
	{
		LOG_INF("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  IMX355_OTP_ERROR_CODE:%d \n", *feature_data, IMX355_OTP_ERROR_CODE);
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
				*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = IMX355_OTP_ERROR_CODE;
				memcpy(feature_data+2, sn_inf_sub_imx355, sizeof(MUINT32)*13); 
				memcpy(feature_data+10, material_inf_sub_imx355, sizeof(MUINT32)*4);
			}
			break;
		}
		break;
