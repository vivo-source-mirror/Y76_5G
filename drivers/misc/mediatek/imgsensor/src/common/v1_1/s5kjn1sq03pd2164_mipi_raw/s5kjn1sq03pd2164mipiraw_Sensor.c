 ...
 * Filename:
 * ---------
 *	 s5kjn1sq03pd2164mipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "s5kjn1sq03pd2164mipiraw_freq.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJN1SQ03PD2164_SENSOR_ID,
...
extern otp_error_code_t S5KJN1SQ03PD2164_OTP_ERROR_CODE;
MUINT32  af_calib_inf_main_s5kjn1sq03pd2164[6];
#endif
MUINT32  sn_inf_main_s5kjn1sq03pd2164[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_s5kjn1sq03pd2164[4];
MUINT32  crosstalk_data_PD2164[8006];  /*0 flag   1-836 data*/
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	kal_uint8 retry = 2;
 	int offset = 0xF;
	unsigned char material_inf_main_s5kjn1sq03[4];
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = read_cmos_sensor_16_16(0x0000) ;
			...
			for(j = 0;j < 4;j++){
					vivo_read_eeprom(offset,&material_inf_main_s5kjn1sq03[j]);	
					offset++;
				}			
				if((int)material_inf_main_s5kjn1sq03[3] == 0x46) {
...
#ifdef HOPE_ADD_FOR_CAM_TEMPERATURE_NODE  //add for s5kjn1sq03pd2164 temperature 
    ...
    */
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = S5KJN1SQ03PD2164_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_s5kjn1sq03pd2164, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main_s5kjn1sq03pd2164, sizeof(MUINT32)*4); 
				memcpy( feature_data+12, af_calib_inf_main_s5kjn1sq03pd2164, sizeof(MUINT32)*6);
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_s5kjn1sq03pd2164[%d]= 0x%x\n", i, sn_inf_main_s5kjn1sq03pd2164[i]);
						}
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:/*get 4 cell data from eeprom*/
	{
		...
		if (type == FOUR_CELL_CAL_TYPE_XTALK_CAL) {
			...
			data[1] = 0x1F;
			for(i = 0; i< 8006 ; i++)
			data[2 + i] = crosstalk_data_PD2164[i];
...
UINT32 S5KJN1SQ03PD2164_MIPI_RAW_SensorInit(
