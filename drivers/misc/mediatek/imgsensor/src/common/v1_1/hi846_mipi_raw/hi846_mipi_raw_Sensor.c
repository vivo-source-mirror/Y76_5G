...
extern otp_error_code_t MAIN3_OTP_ERROR_CODE_HI846;
MUINT32  sn_inf_main2_hi846[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main2_hi846[4];
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    if (timeout) {
    	 pr_err(PFX "[%s] check_i2c_timeout timeout =null \n",__FUNCTION__,timeout);
			...
			do {
				*sensor_id = return_sensor_id();
				...
				if (*sensor_id == imgsensor_info.sensor_id) {				
					...
					LOG_INF("cfx_add:end read eeprom ---vivo_main3_otp_read_when_power_on = %d,MAIN3_OTP_ERROR_CODE_HI846 =%d\n", vivo_main3_otp_read_when_power_on,MAIN3_OTP_ERROR_CODE_HI846);
					/*vivo caofeixiang add end*/
					return ERROR_NONE;
				}	
				pr_err("get_imgsensor_id Read sensor id fail, i2c write id: 0x%x,sensor id: 0x%x  imgsensor_info.sensor_id =0x%x\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
				retry--;
			} while(retry > 0);
			i++;
			retry = 2;
...
static kal_uint32 open(void)
{
    ...
    pr_info(PFX "[%s ] [open]: PLATFORM:VIVO,MIPI 24LANE\n",__FUNCTION__);
...
//    unsigned long long *feature_return_para=(unsigned long long *) feature_para;
	...
	break;
        case SENSOR_FEATURE_GET_CUSTOM_INFO:
		    LOG_INF("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  MAIN3_OTP_ERROR_CODE_HI846:%d \n", *feature_data,MAIN3_OTP_ERROR_CODE_HI846);
			switch (*feature_data) {
				...
				if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
				    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = MAIN3_OTP_ERROR_CODE_HI846;//otp_state
					memcpy( feature_data+2, sn_inf_main2_hi846, sizeof(MUINT32)*13); 
					memcpy( feature_data+10, material_inf_main2_hi846, sizeof(MUINT32)*4); 
					#if 0
							for (i = 0 ; i<12 ; i++ ){
							printk("sn_inf_main2_hi846[%d]= 0x%x\n", i, sn_inf_main2_hi846[i]);
							}
