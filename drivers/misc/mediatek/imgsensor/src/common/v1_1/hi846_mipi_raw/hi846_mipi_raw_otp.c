...
#define VIVO_MAIN3_EEPROM_WRITE_ID    0xA0	 //0xA0
...
extern MUINT32  material_inf_main2_hi846[4];
...
int vivo_main3_otp_read(void)
{
	...
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		/****ModuleInfo****/
		if (  (VIVO_MAIN3_VENDOR_SUNNY != main3_otp_data_hi846[0x01])) {
			...
			return 0;
		} else if ((VIVO_MAIN3_VENDOR_VCM_ID != main3_otp_data_hi846[0x0009]) || (VIVO_MAIN3_VENDOR_DRIVERIC_ID != main3_otp_data_hi846[0x000A]) || (VIVO_MAIN3_VENDOR_PLATFORM_ID != main3_otp_data_hi846[0x0002])) {
			MAIN3_OTP_ERROR_CODE_HI846 = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF(": Platform ID or Lens or VCM ID or Driver_IC ID  Error!!!    otp_error_code:%d\n", MAIN3_OTP_ERROR_CODE_HI846);
			return 0;
		} else if ((VIVO_MAIN3_VENDOR_LENS_ID != main3_otp_data_hi846[0x0008]) && (VIVO_MAIN3_VENDOR_LENS_ID_SUNNY != main3_otp_data_hi846[0x0008])) {
			MAIN3_OTP_ERROR_CODE_HI846 = OTP_ERROR_CODE_MODULE_INFO_ERROR;
		...
		/****material info start****/
			material_inf_main2_hi846[0] = (MUINT32)main3_otp_data_hi846[0x0F];
			material_inf_main2_hi846[1] = (MUINT32)main3_otp_data_hi846[0x10];
			material_inf_main2_hi846[2] = (MUINT32)main3_otp_data_hi846[0x11];
			material_inf_main2_hi846[3] = (MUINT32)main3_otp_data_hi846[0x12];
			LOG_INF("material_inf_main2_hi846[0] = 0x%x, material_inf_main2_hi846[1] = 0x%x, material_inf_main2_hi846[2] = 0x%x,material_inf_main2_hi846[3] = 0x%x\n",
				material_inf_main2_hi846[0]  , material_inf_main2_hi846[1],  material_inf_main2_hi846[2], material_inf_main2_hi846[3]);
		/****material info end****/
		...
		#endif
		LOG_INF("LSC_ main3_otp_data_hi846[i]=0x%x \n",main3_otp_data_hi846[Lsc_addr]);		
		/****LSC_checksum****/
		checksum = 0;
		for (i = Lsc_addr+1; i < Lsc_checksum_addr; i++) {
			checksum += main3_otp_data_hi846[i];
			LOG_INF("LSC_ i=0x%x checksum=0x%x  main3_otp_data_hi846[i]=0x%x \n",i, checksum,main3_otp_data_hi846[i]);
			}
			checksum = checksum % 0xff+1;
			
		if (main3_otp_data_hi846[Lsc_checksum_addr] != checksum) {
			MAIN3_OTP_ERROR_CODE_HI846 = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("LSC_ checksum=0x%x  main3_otp_data_hi846[Lsc_checksum_addr]=0x%x \n",checksum,main3_otp_data_hi846[Lsc_checksum_addr]);
