...
#include "imgsensor.h"
#include "s5kjn1sq03pd2159mipiraw_Sensor.h"
...
unsigned char vivo_otp_data_s5kjn1sq03pd2159[VIVO_OTP_DATA_SIZE];
...
#endif
static int checksum;
otp_error_code_t S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
extern MUINT32  sn_inf_main_s5kjn1sq03pd2159[13];
extern MUINT32  material_inf_main_s5kjn1sq03pd2159[4];  
extern MUINT32 crosstalk_data_PD2159[PDAF_DATA_SIZE - 2];  /* 0 flag   1-606 data*/
...
int MAIN_38E1_otp_read(void)
{
	...
	S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	/* This operation takes a long time, we need to skip it. guojunzheng add begin */
	#if 1
	if (is_atboot == 1) {
		LOG_INF("[lxd++]AT mode skip s5kjn1sq03pd2159_otp_read\n");
		return 1;
	}
	#endif
	/* guojunzheng add end */
	...
	for (i = 0; i < VIVO_OTP_DATA_SIZE; i++) {
		if (!vivo_read_eeprom(offset,  &vivo_otp_data_s5kjn1sq03pd2159[i])) {
			LOG_INF("[lxd++]read_vivo_eeprom 0x%0x %d fail \n", offset,  vivo_otp_data_s5kjn1sq03pd2159[i]);
			return 0;
		}
	...
	#if 0
	for(i = 0; i < VIVO_OTP_DATA_SIZE; i++) {
		LOG_INF("[lxd++]read_vivo_eeprom Data[0x%0x]:0x%x\n", i,  vivo_otp_data_s5kjn1sq03pd2159[i]);
	}
	#endif
	/*check_if_group_valid = vivo_otp_data_s5kjn1sq03pd2159[ModuleInfo_addr] | vivo_otp_data_s5kjn1sq03pd2159[Awb_addr] | vivo_otp_data_s5kjn1sq03pd2159[Af_addr] | vivo_otp_data_s5kjn1sq03pd2159[Lsc_addr] | vivo_otp_data_s5kjn1sq03pd2159[PD_Proc1_addr] | vivo_otp_data_s5kjn1sq03pd2159[PD_Proc2_addr];*/
	if ((0x01 == vivo_otp_data_s5kjn1sq03pd2159[ModuleInfo_addr]) && 
		(0x01 == vivo_otp_data_s5kjn1sq03pd2159[Fulse_id_addr]) && 
		(0x01 == vivo_otp_data_s5kjn1sq03pd2159[SN_addr]) && 
		(0x01 == vivo_otp_data_s5kjn1sq03pd2159[Af_addr]) && 
		(0x01 == vivo_otp_data_s5kjn1sq03pd2159[Awb_addr]) &&
		(0x01 == vivo_otp_data_s5kjn1sq03pd2159[Lsc_addr]) &&
		(0x01 == vivo_otp_data_s5kjn1sq03pd2159[PDAF_proc1_addr]) &&
		(0x01 == vivo_otp_data_s5kjn1sq03pd2159[PDAF_proc2_addr]) &&
		(0x01 == vivo_otp_data_s5kjn1sq03pd2159[XTC_cal_addr]) &&
		(0x01 == vivo_otp_data_s5kjn1sq03pd2159[SensorXTC_cal_addr]) &&
		(0x01 == vivo_otp_data_s5kjn1sq03pd2159[PDXTC_addr]) &&
		(0x01 == vivo_otp_data_s5kjn1sq03pd2159[SW_GGC_addr]) &&
		(0x01 == vivo_otp_data_s5kjn1sq03pd2159[HWGGC_addr]) /*&&
		(0x01 == vivo_otp_data_s5kjn1sq03pd2159[OIS_shift_calibration_addr])*/) {
	...
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		/****ModuleInfo****/
		if ((VIVO_VENDOR_QTECH != vivo_otp_data_s5kjn1sq03pd2159[0x01]) ) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("[lxd++]Module ID error!!!    otp_error_code:%d\n", S5KJN1SQ03PD2159_OTP_ERROR_CODE);
			return 0;
		} else if ((VIVO_VENDOR_LENS_ID != vivo_otp_data_s5kjn1sq03pd2159[0x08]) || 
				(VIVO_VENDOR_VCM_ID != vivo_otp_data_s5kjn1sq03pd2159[0x09]) || 
				(VIVO_VENDOR_DRIVERIC_ID != vivo_otp_data_s5kjn1sq03pd2159[0x0A]) || 
				(VIVO_VENDOR_PLATFORM_ID != vivo_otp_data_s5kjn1sq03pd2159[0x02])) {
				
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("[lxd++]: Platform ID or Lens or VCM ID or Driver_IC ID  Error!!!    otp_error_code:%d\n", S5KJN1SQ03PD2159_OTP_ERROR_CODE);
			return 0;
		} else if ((0xff != vivo_otp_data_s5kjn1sq03pd2159[0x0B]) || 
				(0x00 != vivo_otp_data_s5kjn1sq03pd2159[0x0C]) || 
				(0x0b != vivo_otp_data_s5kjn1sq03pd2159[0x0D]) || 
				((0x01 != vivo_otp_data_s5kjn1sq03pd2159[0x0E]) && (0x03 != vivo_otp_data_s5kjn1sq03pd2159[0x0E]) && (0x02 != vivo_otp_data_s5kjn1sq03pd2159[0x0E]))) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("[lxd++]: calibration version  Error!!!    Read version:0x%2x%2x%2x%2x\n", 
				vivo_otp_data_s5kjn1sq03pd2159[0x0B], vivo_otp_data_s5kjn1sq03pd2159[0x0C], vivo_otp_data_s5kjn1sq03pd2159[0x0D], vivo_otp_data_s5kjn1sq03pd2159[0x0E]);
			return 0;
		}
		/****ModuleInfo_checksum****/
		checksum = 0;
		for (i = ModuleInfo_addr+1; i < ModuleInfo_checksum_addr; i++) {
			checksum += vivo_otp_data_s5kjn1sq03pd2159[i];
			}
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_s5kjn1sq03pd2159[ModuleInfo_checksum_addr] != checksum) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("[lxd++]ModuleInfo_checksum error!!!   otp_error_code:%d\n", S5KJN1SQ03PD2159_OTP_ERROR_CODE);
			return 0;
			}
		
		/****material info start****/
			material_inf_main_s5kjn1sq03pd2159[0] = (MUINT32)vivo_otp_data_s5kjn1sq03pd2159[0x0F];
			material_inf_main_s5kjn1sq03pd2159[1] = (MUINT32)vivo_otp_data_s5kjn1sq03pd2159[0x10];
			material_inf_main_s5kjn1sq03pd2159[2] = (MUINT32)vivo_otp_data_s5kjn1sq03pd2159[0x11];
			material_inf_main_s5kjn1sq03pd2159[3] = (MUINT32)vivo_otp_data_s5kjn1sq03pd2159[0x12];
			LOG_INF("material_inf_main_s5kjn1sq03pd2159[0] = 0x%x, material_inf_main_s5kjn1sq03pd2159[1] = 0x%x, material_inf_main_s5kjn1sq03pd2159[2] = 0x%x,material_inf_main_s5kjn1sq03pd2159[3] = 0x%x\n",
				material_inf_main_s5kjn1sq03pd2159[0]  , material_inf_main_s5kjn1sq03pd2159[1],  material_inf_main_s5kjn1sq03pd2159[2], material_inf_main_s5kjn1sq03pd2159[3]);
		/****material info end****/
		...
		for (i = Fulse_id_addr+1; i < Fulse_id_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_s5kjn1sq03pd2159[Fulse_id_checksum_addr] != checksum) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = SN_addr+1; i < SN_checksum_addr; i++) {
			/*LOG_INF("vivo_otp_data_s5kjn1sq03pd2159[0x%x] = 0x%x\n", i, vivo_otp_data_s5kjn1sq03pd2159[i]);*/
			checksum += vivo_otp_data_s5kjn1sq03pd2159[i];
			}
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_s5kjn1sq03pd2159[SN_checksum_addr] != checksum) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("SN_checksum error!!!   otp_error_code:%d\n", S5KJN1SQ03PD2159_OTP_ERROR_CODE);
			return 0;
		}
		sn_inf_main_s5kjn1sq03pd2159[0] = 0x01;
		for (i = 0; i < 12; i++) {
			sn_inf_main_s5kjn1sq03pd2159[i+1] = (MUINT32)vivo_otp_data_s5kjn1sq03pd2159[i + SN_addr+1];
			/*LOG_INF("sn_inf_main_s5kjn1sq03pd2159[%d] = 0x%x, vivo_otp_data_s5kjn1sq03pd2159[0x%x] = 0x%x\n", i+1  , sn_inf_main_s5kjn1sq03pd2159[i+1],  i +SN_addr+1, vivo_otp_data_s5kjn1sq03pd2159[i+SN_addr+1]);*/
		}
		...
		for (i = Af_addr+1; i < Af_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_s5kjn1sq03pd2159[Af_checksum_addr] != checksum) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = Awb_addr+1; i < Awb_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_s5kjn1sq03pd2159[Awb_checksum_addr] != checksum) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = Lsc_addr+1; i < Lsc_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_s5kjn1sq03pd2159[Lsc_checksum_addr] != checksum) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = PDAF_proc1_addr+1; i < PDAF_proc1_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_s5kjn1sq03pd2159[PDAF_proc1_checksum_addr] != checksum) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("[lxd++]PDAF_data_checksum error!!!   otp_error_code:%d\n", S5KJN1SQ03PD2159_OTP_ERROR_CODE);
			return 0;
			}
		...
		/****PDAF Proc2_checksum****/
		if (0x01 == vivo_otp_data_s5kjn1sq03pd2159[PDAF_proc2_addr]){
			checksum = 0;
			for (i = PDAF_proc2_addr+1; i < PDAF_proc2_checksum_addr; i++) {
				checksum += vivo_otp_data_s5kjn1sq03pd2159[i];
			}
				checksum = checksum % 0xff+1;
			if (vivo_otp_data_s5kjn1sq03pd2159[PDAF_proc2_checksum_addr] != checksum) {
				S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
				LOG_INF("[lxd++]PDAF_data_checksum error!!!   otp_error_code:%d\n", S5KJN1SQ03PD2159_OTP_ERROR_CODE);
				return 0;
				}
		...
		for (i = XTC_cal_addr+1; i < XTC_cal_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_s5kjn1sq03pd2159[XTC_cal_checksum_addr] != checksum) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = SensorXTC_cal_addr+1; i < SensorXTC_cal_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_s5kjn1sq03pd2159[SensorXTC_cal_checksum_addr] != checksum) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("XTC_cal_checksum_addr  error!!!   otp_error_code:%d\n", S5KJN1SQ03PD2159_OTP_ERROR_CODE);
			return 0;
			}
		...
		for (i = PDXTC_addr+1; i < PDXTC_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_s5kjn1sq03pd2159[PDXTC_checksum_addr] != checksum) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = SW_GGC_addr+1; i < SW_GGC_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_s5kjn1sq03pd2159[SW_GGC_checksum_addr] != checksum) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("SW_GGC_checksum_addr  %x vs %x\n", checksum, vivo_otp_data_s5kjn1sq03pd2159[SW_GGC_checksum_addr]);
		...
		for (i = 0; i < PDAF_DATA_SIZE - 2; i++) {
			...
			crosstalk_data_PD2159[i]=vivo_otp_data_s5kjn1sq03pd2159[XTC_cal_addr + i + index];
		}
		...
		for (i = HWGGC_addr+1; i < HWGGC_addr_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_s5kjn1sq03pd2159[HWGGC_addr_checksum_addr] != checksum) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = OIS_addr+1; i < OIS_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
			LOG_INF("checksum :%d  OIS_checksum=%d\n", checksum,vivo_otp_data_s5kjn1sq03pd2159[OIS_checksum_addr]);

		if (vivo_otp_data_s5kjn1sq03pd2159[OIS_checksum_addr] != checksum) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = OIS_shift_calibration_addr+1; i < OIS_shift_calibration_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_s5kjn1sq03pd2159[OIS_shift_calibration_checksum_addr] != checksum) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		R_unit = vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+1];
		R_unit = (R_unit << 8) | (vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+2]);
		B_unit = vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+3];
		B_unit = (B_unit << 8) | (vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+4]);
		G_unit = vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+5];
		G_unit = (G_unit << 8) | (vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+6]);

		R_golden = vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+7];
		R_golden = (R_golden << 8) | (vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+8]);
		B_golden = vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+9];
		B_golden = (B_golden << 8) | (vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+10]);
		G_golden = vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+11];
		G_golden = (G_golden << 8) | (vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+12]);
		...
		if (t > 0) {
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("[lxd++]AWB[high cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, S5KJN1SQ03PD2159_OTP_ERROR_CODE);
			return 0;
		}

		
		/****check if awb out of range[3000K low cct]****/
		R_unit_low = vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+13];
		R_unit_low = (R_unit_low << 8) | (vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+14]);
		B_unit_low = vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+15];
		B_unit_low = (B_unit_low << 8) | (vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+16]);
		G_unit_low = vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+17];
		G_unit_low = (G_unit_low << 8) | (vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+18]);

		R_golden_low = vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+19];
		R_golden_low = (R_golden_low << 8) | (vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+20]);
		B_golden_low = vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+21];
		B_golden_low = (B_golden_low << 8) | (vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+22]);
		G_golden_low = vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+23];
		G_golden_low = (G_golden_low << 8) | (vivo_otp_data_s5kjn1sq03pd2159[Awb_addr+24]);
		...
		AF_inf  =  vivo_otp_data_s5kjn1sq03pd2159[Af_addr + 9];
		AF_inf = (AF_inf << 8) | (vivo_otp_data_s5kjn1sq03pd2159[Af_addr + 10]);

		AF_mac = vivo_otp_data_s5kjn1sq03pd2159[Af_addr + 11];
		AF_mac = (AF_mac << 8) | (vivo_otp_data_s5kjn1sq03pd2159[Af_addr + 12]);
		...
		if (diff_inf_macro > 480 || diff_inf_macro < 200) {  /*AF code out of range*/
			S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
			pr_err("[%s] [lxd++]AF out of range error!!!   otp_error_code:%d    Module ID = 0x%0x   inf = %d    mac = %d\n", __FUNCTION__,S5KJN1SQ03PD2159_OTP_ERROR_CODE, vivo_otp_data_s5kjn1sq03pd2159[0x01], AF_inf, AF_mac);
			return 0;
		}
		#endif
		...
		{
			if(i == 0)
				LOG_INF("[lxd++]read_S5KJN1SQ03PD2159_PDAF_data");
			if(i < 496)
				PDAF_data[i] = vivo_otp_data_s5kjn1sq03pd2159[PD_Proc1_addr+i+1];
			else //i >= 496
				PDAF_data[i] = vivo_otp_data_s5kjn1sq03pd2159[PD_Proc1_addr+i+3];
		}*/
		/*copy pdaf data*/
		/*memcpy(PDAF_data, &vivo_otp_data_s5kjn1sq03pd2159[PDAF_addr+1], PDAF_checksum_addr-PDAF_addr-1);*/
		/*lxd add for pdaf data end*/
		S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
		pr_err("[%s]otp read normally return\n");
		return 1;
	} else {
		S5KJN1SQ03PD2159_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
		...
		PDAF_proc1_addr = 0x%x, PDAF_proc2_addr = 0x%x, XTC_cal_addr = 0x%x, SensorXTC_cal_addr = 0x%x, PDXTC_addr = 0x%x, SW_GGC_addr = 0x%x, OIS_addr = 0x%x\n",
             __FUNCTION__,
		vivo_otp_data_s5kjn1sq03pd2159[ModuleInfo_addr],
		vivo_otp_data_s5kjn1sq03pd2159[Fulse_id_addr],
		vivo_otp_data_s5kjn1sq03pd2159[SN_addr],
		vivo_otp_data_s5kjn1sq03pd2159[Af_addr],
		vivo_otp_data_s5kjn1sq03pd2159[Awb_addr],
		vivo_otp_data_s5kjn1sq03pd2159[Lsc_addr],
		vivo_otp_data_s5kjn1sq03pd2159[PDAF_proc1_addr],
		vivo_otp_data_s5kjn1sq03pd2159[PDAF_proc2_addr],
		vivo_otp_data_s5kjn1sq03pd2159[XTC_cal_addr],
		vivo_otp_data_s5kjn1sq03pd2159[SensorXTC_cal_addr],
		vivo_otp_data_s5kjn1sq03pd2159[PDXTC_addr],
		vivo_otp_data_s5kjn1sq03pd2159[SW_GGC_addr],
		vivo_otp_data_s5kjn1sq03pd2159[HWGGC_addr]);
		pr_err("[%s][lxd++]invalid otp data. error!!!   otp_error_code:%d\n", __FUNCTION__,S5KJN1SQ03PD2159_OTP_ERROR_CODE);
		return 0;
	}
}
/*vivo lxd add end*/
