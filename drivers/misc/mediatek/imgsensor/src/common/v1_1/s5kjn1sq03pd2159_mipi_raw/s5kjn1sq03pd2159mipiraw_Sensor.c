 ...
 * Filename:
 * ---------
 *	 s5kjn1sq03pd2159mipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "s5kjn1sq03pd2159mipiraw_freq.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJN1SQ03PD2159_SENSOR_ID,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {
	...
	{8160, 6144,  432,  336, 7296, 5472, 3648, 2736, 0, 0, 3648, 2736, 0, 0, 3648, 2736}, /* custom1 */
	...
	{8160, 6144,  240,  912, 7680, 4320, 3840, 2160, 0, 0, 3840, 2160, 0, 0, 3840, 2160}, /* custom5 */
};
#if 1
...
{/* Preview mode setting 4080*3060*/
	...
	/* custom1 mode setting 3648*2736 30fps*/
	{0x02, 0x0a, 0x00, 0x08, 0x40, 0x00,
	 0x00, 0x2b, 0x1200, 0x0A20, 0x00, 0x00, 0x0000, 0x0000,
	 0x01, 0x30, 0x023A, 0x0AA0, 0x00, 0x00, 0x0000, 0x0000},
};
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info =  //4080X3072
{
	...
	.i4BlockNumY = 382,
	.iMirrorFlip = 3,
	.i4Crop = { {0,0}, {0,0}, {0, 382}, {0, 0}, {0, 0}, {216,162}, {120, 450}, {0, 0}, {0, 0}, {120, 450}},
};
#endif
#if 1
...
extern otp_error_code_t S5KJN1SQ03PD2159_OTP_ERROR_CODE;
#endif
MUINT32  sn_inf_main_s5kjn1sq03pd2159[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_s5kjn1sq03pd2159[4];
MUINT32  crosstalk_data_PD2159[PDAF_DATA_SIZE - 2];  /* 0 flag   1-836 data* */
...
0x0312, 0x0000,
0x080E, 0x0000,
0x0340, 0x0C70,  ////Continue 0x0FF8  NO-Continue 0x0C70
0x0342, 0x1700,  ////Continue 0x11E8  NO-Continue 0x0C70
0x0702, 0x0000,
0x0202, 0x0100,
0x0200, 0x0100,
0x0D00, 0x0101,
...
0x0D04, 0x0102,
0x6226, 0x0000,	
0x011E, 0x0000,  ////Continue 0x0100  NO-Continue 0x0000
	};
...
0x0D04, 0x0102,
0x6226, 0x0000,
0x011E, 0x0100,  ////Continue 0x0100  NO-Continue 0x0000	
	};
...
0x0312, 0x0000,
0x080E, 0x0000,
0x0340, 0x0948,  //////240fps=0x04A4  120fps=0x0948
0x0342, 0x0830,
0x0702, 0x0000,
0x0202, 0x0100,
0x0200, 0x0100,
...
0x0312, 0x0000,
0x080E, 0x0000,
0x0340, 0x08F4,   /////60fps=0x08F4  30fps=0x11E8
0x0342, 0x1000,
0x0702, 0x0000,
0x0202, 0x0100,
0x0200, 0x0100,
...
0x0312, 0x0000,
0x080E, 0x0000,
0x0340, 0x11E8,   /////60fps=0x08F4  30fps=0x11E8
0x0342, 0x1000,
0x0702, 0x0000,
0x0202, 0x0100,
0x0200, 0x0100,
...
#ifdef HOPE_ADD_FOR_CAM_TEMPERATURE_NODE  //add for s5kjn1sq03pd2159 temperature
...
case SENSOR_FEATURE_GET_RAW_INFO:
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		...
		switch (*feature_data) {
			...
			case MSDK_SCENARIO_ID_CUSTOM1:
				imgsensor_pd_info.i4BlockNumX = 456;
				imgsensor_pd_info.i4BlockNumY = 340;
				memcpy((void *)PDAFinfo,(void *)&imgsensor_pd_info,sizeof(struct SET_PD_BLOCK_INFO_T));
				break;
	...
	case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
		...
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 1; /* 4080x2296 */
			break;
		case MSDK_SCENARIO_ID_CUSTOM1:
			*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 1; /* 3648X2736 */
			break;
		case MSDK_SCENARIO_ID_CUSTOM2:
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)((feature_data+1)) = S5KJN1SQ03PD2159_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_s5kjn1sq03pd2159, sizeof(MUINT32)*13);
				memcpy( feature_data+10, material_inf_main_s5kjn1sq03pd2159, sizeof(MUINT32)*4);
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_s5kjn1sq03pd2159[%d]= 0x%x\n", i, sn_inf_main_s5kjn1sq03pd2159[i]);
						}
			}
				break;
			}
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:/* get 4 cell data from eeprom*/
	{
		...
		if (type == FOUR_CELL_CAL_TYPE_XTALK_CAL) {
			...
			for(i = 0; i<  PDAF_DATA_SIZE - 2; i++)
				data[2 + i] = crosstalk_data_PD2159[i];
		}
	}
	break;
...
UINT32 S5KJN1SQ03PD2159_MIPI_RAW_SensorInit(
