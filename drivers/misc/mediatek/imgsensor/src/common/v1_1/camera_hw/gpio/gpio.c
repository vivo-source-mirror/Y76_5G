...
#ifdef CONFIG_REGULATOR_FAN53870
	pgpio->enable_cnt[((IMGSENSOR_HW_PIN_PLUGIC_EN - IMGSENSOR_HW_PIN_PDN) << 1) + GPIO_STATE_H] = 0;
	printk("%s : IMGSENSOR_HW_PIN_PLUGIC_EN enable_cnt=%d\n",__FUNCTION__,pgpio->enable_cnt[((IMGSENSOR_HW_PIN_PLUGIC_EN - IMGSENSOR_HW_PIN_PDN) << 1) + GPIO_STATE_H]);
#endif
	...
	enum IMGSENSOR_HW_PIN_STATE pin_state)
{
	...
	if (ppinctrl_state != NULL && !IS_ERR(ppinctrl_state)){
#ifdef CONFIG_REGULATOR_FAN53870 //for PD2083F_EX fan53870 IC  enable
		if (pin == IMGSENSOR_HW_PIN_PLUGIC_EN) {
			printk("%s : IMGSENSOR_HW_PIN_PLUGIC_EN GPIO_STATE  pin =%d gpio_state =%d enable_cnt=%d\n",__FUNCTION__,pin,gpio_state ,pgpio->enable_cnt[((pin - IMGSENSOR_HW_PIN_PDN) << 1) + GPIO_STATE_H]);		}	
		if (pin == IMGSENSOR_HW_PIN_PLUGIC_EN && gpio_state ==GPIO_STATE_H ) {
		...
		pinctrl_select_state(pgpio->ppinctrl, ppinctrl_state);
				...
				printk("%s : if IMGSENSOR_HW_PIN_PLUGIC_EN GPIO_STATE_H  enable_cnt++=%d\n",__FUNCTION__,pgpio->enable_cnt[((pin - IMGSENSOR_HW_PIN_PDN) << 1) + GPIO_STATE_H]);
			}else {
				pgpio->enable_cnt[((pin - IMGSENSOR_HW_PIN_PDN) << 1) + GPIO_STATE_H] ++;
				printk("%s : else IMGSENSOR_HW_PIN_PLUGIC_EN GPIO_STATE_H  enable_cnt++=%d\n",__FUNCTION__,pgpio->enable_cnt[((pin - IMGSENSOR_HW_PIN_PDN) << 1) + GPIO_STATE_H]);
			}
		} else if (pin == IMGSENSOR_HW_PIN_PLUGIC_EN && gpio_state ==GPIO_STATE_L ) {
			if (pgpio->enable_cnt[((pin - IMGSENSOR_HW_PIN_PDN) << 1) + GPIO_STATE_H] > 1) {
				pgpio->enable_cnt[((pin - IMGSENSOR_HW_PIN_PDN) << 1) + GPIO_STATE_H] --;
				printk("%s : if IMGSENSOR_HW_PIN_PLUGIC_EN GPIO_STATE_L  enable_cnt=%d\n",__FUNCTION__,pgpio->enable_cnt[((pin - IMGSENSOR_HW_PIN_PDN) << 1) + GPIO_STATE_H]);
			} else {
				pinctrl_select_state(pgpio->ppinctrl, ppinctrl_state);
				pgpio->enable_cnt[((pin - IMGSENSOR_HW_PIN_PDN) << 1) + GPIO_STATE_H] --;
				printk("%s : else IMGSENSOR_HW_PIN_PLUGIC_EN GPIO_STATE_L enable_cnt=%d\n",__FUNCTION__,pgpio->enable_cnt[((pin - IMGSENSOR_HW_PIN_PDN) << 1) + GPIO_STATE_H]);
			}
		}else{
			pinctrl_select_state(pgpio->ppinctrl, ppinctrl_state);
		}
