...
extern MUINT32 crosstalk_data_PD2069[8006];  /*0 flag   1-606 data*/
...
int MAIN_38E1_otp_read(void)
{
	...
	for (i = 0; i < VIVO_OTP_DATA_SIZE; i++) {
		if (!vivo_read_eeprom(offset,  &vivo_otp_data_s5kjn1sq03[i])) {
			LOG_INF("[lxd++]read_vivo_eeprom 0x%0x %d fail \n", offset,  vivo_otp_data_s5kjn1sq03[i]);
			return 0;
		}
	...
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		...
		// memcpy(crosstalk_data_PD2069, &vivo_otp_data_s5kjn1sq03[0x1a40], 2612);
		for (i = 0; i < 2612; i++) {
			crosstalk_data_PD2069[i]=vivo_otp_data_s5kjn1sq03[0x1a40+i];
		}
		/****XTC_cal_checksum****/
		checksum = 0;
		...
		// memcpy(&crosstalk_data_PD2069[2612], &vivo_otp_data_s5kjn1sq03[0x2476], 768);
		for (i = 0; i < 768; i++) {
			crosstalk_data_PD2069[2612+i]=vivo_otp_data_s5kjn1sq03[0x2476+i];
		}
		/****PDXTC_checksum****/
		checksum = 0;
		...
		// memcpy(&crosstalk_data_PD2069[3380], &vivo_otp_data_s5kjn1sq03[0x2778], 4000);
		for (i = 0; i < 4000; i++) {
			crosstalk_data_PD2069[3380+i]=vivo_otp_data_s5kjn1sq03[0x2778+i];
		}
		/****SW_GGC_checksum****/
		checksum = 0;
		...
		//memcpy(&crosstalk_data_PD2069[7380], &vivo_otp_data_s5kjn1sq03[0x371A], 626);
		for (i = 0; i < 626; i++) {
			crosstalk_data_PD2069[7380+i]=vivo_otp_data_s5kjn1sq03[0x371A+i];
		}

		for (i = 0; i < 8006; i++) {
			LOG_INF("crosstalk_data_PD2069[%d] =0x%x\n", i,crosstalk_data_PD2069[i]);
		}
		...
		for (i = OIS_addr+1; i < OIS_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
			LOG_INF("checksum :%d  OIS_checksum=%d\n", checksum,vivo_otp_data_s5kjn1sq03[OIS_checksum_addr]);

		if (vivo_otp_data_s5kjn1sq03[OIS_checksum_addr] != checksum) {
			S5KJN1SQ03_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = OIS_shift_calibration_addr+1; i < OIS_shift_calibration_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_s5kjn1sq03[OIS_shift_calibration_checksum_addr] != checksum) {
			S5KJN1SQ03_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
