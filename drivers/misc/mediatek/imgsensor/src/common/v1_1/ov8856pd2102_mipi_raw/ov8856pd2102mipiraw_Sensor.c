...
//#include "ov8856pd2102mipiraw_freq.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV8856PD2102_SENSOR_ID,
	...
	.pre = {
		...
		.framelength = 2558,	/*record different mode's framelength*/
...
extern int vivo_main3_otp_read_ov8856pd2102(void);
extern otp_error_code_t MAIN3_OTP_ERROR_CODE_OV8856pd2102;
MUINT32  sn_inf_main2_ov8856pd2102[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main2_ov8856pd2102[4];  
...
static void set_shutter(kal_uint32 shutter)
{
	...
	if(shutter > 65528) {  //linetime=10160/960000000<< maxshutter=3023622-line=32s
		...
		pr_info("enter long exposure mode shutter = %d, imgsensor.line_length =%d\n", shutter, imgsensor.line_length);
		
		if(shutter > (22*65528)){
			new_line_length = shutter / 65528 * imgsensor.line_length;
		}
		else{
			new_line_length = shutter * imgsensor.line_length / 65528 ;
		}
		shutter = 65528;
		pr_info("long exposure change: shutter = %d, new_line_length =%d\n", shutter, new_line_length);
		if(new_line_length > 0xffff){
			new_line_length = 0xffff; //max value
		}
		...
		PK_DBG("enter normal exposure mode\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = ((read_cmos_sensor_16_8(0x300B) << 8) | read_cmos_sensor_16_8(0x300C)) + 2;
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				vivo_otp_read_when_power_on = vivo_main3_otp_read_ov8856pd2102();
				pr_debug("read eeprom ---vivo_otp_read_when_power_on = %d, MAIN2_885A_OTP_ERROR_CODE = %d\n", vivo_otp_read_when_power_on, MAIN3_OTP_ERROR_CODE_OV8856pd2102);
...
{
	pr_err("info: ov8856pd2102 close");
			   ...
			   MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	//ov8856 mclk by hw version start
	/*k6833v1_64_PD2102F_EX.dts boards_version
	  gpios = <45 46 65 69 72 73 74 75 76 171>;
	  We need check gpio 72(gpios[4]): 1-4mA 0-2mA
	*/
	hw_version = get_board_version();
	if(hw_version){
		pr_err("info:ov8856mclk get_board_version()=%s, hv[4]=%c\n",
		    hw_version, hw_version[4]);
		if(hw_version[4] == '0'){//gpio 72 is low, set 2mA
			pr_err("info:ov8856mclk:set 2mA\n");
			sensor_info->SensorDrivingCurrent = ISP_DRIVING_2MA;
		}
		else{
			pr_err("info:ov8856mclk:set 4mA\n");
		}
	}
	else{
		pr_err("ov8856mclk:get_board_version()=NULL\n");
	}
	//ov8856 mclk by hw version end
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	pr_err("info:ov8856pd2102 scenario_id = %d\n", scenario_id);
	spin_unlock(&imgsensor_drv_lock);
	switch (scenario_id) {
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
	...
	enum MSDK_SCENARIO_ID_ENUM scenario_id,	MUINT32 framerate)
{
	...
	pr_err("info:ov8856pd2102 scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = MAIN3_OTP_ERROR_CODE_OV8856pd2102;//otp_state
				memcpy( feature_data+2, sn_inf_main2_ov8856pd2102, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main2_ov8856pd2102, sizeof(MUINT32)*4); 
				#if 0
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main2_ov8856pd2102[%d]= 0x%x\n", i, sn_inf_main2_ov8856pd2102[i]);
						}
				#endif
			}
				break;
...
UINT32 OV8856PD2102_MIPI_RAW_SensorInit(
