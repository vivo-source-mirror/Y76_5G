...
extern int MAIN3_2e0_otp_read(void);
extern otp_error_code_t GC02M1PD2159_OTP_ERROR_CODE;
#endif
MUINT32  sn_inf_main3_gc02m1pd2159[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main3_gc02m1pd2159[4];
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_id = GC02M1PD2159_SENSOR_ID,
...
static kal_uint32 return_sensor_id(void)
{
	...
	if (tmp_id == GC02M1PD2159_SENSOR_ID) {
		LOG_INF("sensor id 02e0\n");
		...
		do {
			*sensor_id = return_sensor_id();
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				vivo_otp_read_when_power_on = MAIN3_2e0_otp_read();
				pr_debug("read eeprom ---vivo_otp_read_when_power_on = %d,MAIN3_2E0_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, GC02M1PD2159_OTP_ERROR_CODE);
				/*vivo yy add end*/
				return ERROR_NONE;
			}
			LOG_INF
				  ...
				  UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
	    printk("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  MAIN3_2E0_OTP_ERROR_CODE:%d \n", *feature_data,GC02M1PD2159_OTP_ERROR_CODE);
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)((feature_data+1)) = GC02M1PD2159_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main3_gc02m1pd2159, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main3_gc02m1pd2159, sizeof(MUINT32)*4); 
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main3_gc02m1pd2159[%d]= 0x%x\n", i, sn_inf_main3_gc02m1pd2159[i]);
						}
			}
				break;
			}
...
UINT32 GC02M1PD2159_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
