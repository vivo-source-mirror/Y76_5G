 ...
 * Filename:
 * ---------
 *	 s5kgw3sp13pd2069mipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "s5kgw3sp13pd2069mipiraw_freq.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KGW3SP13PD2069_SENSOR_ID,
	...
	.slim_video = { /* hs_video 240fps*/
		...
		.grabwindow_height = 1080,
		.mipi_data_lp2hs_settle_dc = 85,
		.mipi_pixel_rate = 691600000, //1848
		.max_framerate = 2400,
		},
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {  
	...
	{9216, 6912,  0,  0, 9216, 6912, 9216, 6912, 0, 0, 9216, 6912, 0, 0, 9216, 6912}, /* custom3 *//* sw remosiac*/
	{9216, 6912,  0, 864, 9216, 5184, 4608, 2592, 0, 0, 4608, 2592, 0, 0, 4608, 2592}, /*custom4*/
	{9216, 6912,  768, 1296, 7680, 4320, 3840, 2160, 0, 0, 3840, 2160, 0, 0, 3840, 2160}, 
};
...
static int vivo_otp_read_when_power_on;
  /*0 flag   1-12 data*/
extern int MAIN_7309_otp_read_gw3_pd2069(void);
extern otp_error_code_t S5KGW3SP13PD2069_OTP_ERROR_CODE;


/*hope add otp check end*/
#endif
MUINT32  material_inf_main_s5kgw3sp13pd2069[4];
MUINT32  sn_inf_main_s5kgw3sp13pd2069[13];
MUINT32  af_calib_inf_main_s5kgw3sp13pd2069[6];
...
static kal_uint32 ois_fw_check(UINT8 *params)
{
 wang*/
	...
	if (pi2c_cfg && pi2c_cfg->pinst) {
		...
		ois_interface_dispatcher(AFIOC_X_OIS_INIT, NULL);
		...
		do {
			*sensor_id = read_cmos_sensor_16_16(0x0000) ;
			...
			if ( *sensor_id == imgsensor_info.sensor_id) {
				...
				vivo_otp_read_when_power_on = MAIN_7309_otp_read_gw3_pd2069();
				pr_debug("read eeprom ---vivo_otp_read_when_power_on = %d,MAIN_F8D1_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, S5KGW3SP13PD2069_OTP_ERROR_CODE);
				/*vivo lxd add end*/
				return ERROR_NONE;
			}
...
#ifdef HOPE_ADD_FOR_CAM_TEMPERATURE_NODE  //add for s5kgw3sp13pd2069 temperature 
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)((feature_data+1)) = S5KGW3SP13PD2069_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_s5kgw3sp13pd2069, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main_s5kgw3sp13pd2069, sizeof(MUINT32)*4); 
				memcpy( feature_data+12, af_calib_inf_main_s5kgw3sp13pd2069, sizeof(MUINT32)*6);
				#if 0
						for (i = 0 ; i<12 ; i++ ){
						PK_DBG("sn_inf_main_s5kgw3sp13pd2069[%d]= 0x%x\n", i, sn_inf_main_s5kgw3sp13pd2069[i]);
						}
				#endif
			}
				break;
...
UINT32 S5KGW3SP13PD2069_MIPI_RAW_SensorInit(
