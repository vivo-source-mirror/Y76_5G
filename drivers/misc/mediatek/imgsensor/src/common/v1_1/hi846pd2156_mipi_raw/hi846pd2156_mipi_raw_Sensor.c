...
#define PFX "hi846pd2156_camera_sensor"
#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);

//vivo lishuo add for otp remake 20210823 [B210820-3030] begin
struct IMGSENSOR_SENSOR_TABLEINFO hi846pd2156_otpinfo={	
	.OTP_slave_addr = 0xA2,
	.OTP_size = 0x143F ,
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = HI846PD2156_SENSOR_ID,
	...
	.i2c_addr_table = {0x40, 0x42, /*0x20,*/0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
...
extern otp_error_code_t MAIN3_OTP_ERROR_CODE_HI846PD2156;
MUINT32  sn_inf_main2_hi846pd2156[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_hi846pd2156[4];  
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	kal_uint8 retry = 2;
		...
		kal_uint8 timeout = 0;
		timeout = check_i2c_timeout(0x0846, imgsensor.i2c_write_id);
	    if (timeout) {
	...
	if (pgi2c_cfg_legacy != NULL){
		...
		LOG_INF("hi846pd2156mipiraw_Sensor I2C_BUS = %d\n", I2C_BUS);	
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			do {
				*sensor_id = return_sensor_id();
				printk("sensor_idx i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				if (*sensor_id == imgsensor_info.sensor_id) {				
					LOG_INF("i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
					sensor_init();
					return ERROR_NONE;
				}	
				LOG_INF("get_imgsensor_id Read sensor id fail, i2c write id: 0x%x,sensor id: 0x%x  imgsensor_info.sensor_id =0x%x\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
				retry--;
			} while(retry > 0);
			i++;
			retry = 2;
...
//    unsigned long long *feature_return_para=(unsigned long long *) feature_para;
	...
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 6300000;
		break;
	...
	break;
        case SENSOR_FEATURE_GET_CUSTOM_INFO: //vivo lishuo modify for otp remake 20210823 [B210820-3030]
			switch (*feature_data) {
				case 0:    
					memcpy((MUINT8 *)feature_data,(MUINT8 *)&hi846pd2156_otpinfo,sizeof(struct IMGSENSOR_SENSOR_TABLEINFO));
					break;
				default:
					PK_DBG("SENSOR_FEATURE_GET_CUSTOM_INFO error");
					break;
			}
			break;
...
{
	kal_uint32 i = 0;
	unsigned char pFeaturePara_8[9];
	if(data == NULL){
		return 0;
	}
	//i2c write otp init setting
	write_cmos_sensor_16_8(0x0A02, 0x01);
	write_cmos_sensor_16_8(0x0A00, 0x00);
	mdelay(10);
	write_cmos_sensor_16_8(0x0F02, 0x00);   //PLL disable
	write_cmos_sensor_16_8(0x071A, 0x01);   //CP TRI_H
	write_cmos_sensor_16_8(0x071B, 0x09);   //IPGM TRIM_H
	write_cmos_sensor_16_8(0x0d04, 0x01);   //FSync output enable
	write_cmos_sensor_16_8(0x0d00, 0x07);   //FSync output Drivabiliy
	write_cmos_sensor_16_8(0x003E, 0x10);   //OTP RW

	write_cmos_sensor_16_8(0x0A00, 0x01);   //Sleep Off

	write_cmos_sensor_16_8(0x070a, 0x00);   //start address H
	write_cmos_sensor_16_8(0x070b, 0x00);   //start address L
	write_cmos_sensor_16_8(0x0702, 0x01);   //Single Write	
	
	read_cmos_sensor_16_8(0x0708);
	//read Fuse ID
	for(i = 0;i < 9;i++)
	{
		pFeaturePara_8[i] = read_cmos_sensor_16_8(0x0708);
		PK_DBG("fuseid_inf_sub_hi846pd2057a[%d] = 0x%x",i,pFeaturePara_8[i]);
	}
	sprintf(data,"%02X%02X%02X%02X%02X%02X%02X%02X%02X",
	   pFeaturePara_8[0],pFeaturePara_8[1],pFeaturePara_8[2],
	   pFeaturePara_8[3],pFeaturePara_8[4],pFeaturePara_8[5],
	   pFeaturePara_8[6],pFeaturePara_8[7],pFeaturePara_8[8]);
	//restore normal preview
	write_cmos_sensor_16_8(0x0A00, 0x00);     // stand by on 
	mdelay(10); 
	write_cmos_sensor_16_8(0x003E, 0x00);     // display mode 
	write_cmos_sensor_16_8(0x0A00, 0x01);     // stand by off 
	
	return 0;
}


static MUINT32 hi846pd2156GetOtpInfo(struct IMGSENSOR_SENSOR_TABLEINFO **p){  
    if(p != NULL){
        *p = &hi846pd2156_otpinfo;
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	NULL,
	NULL,
	hi846pd2156GetOtpInfo //vivo lishuo add for otp remake 20210823 [B210820-3030]
};

UINT32 HI846PD2156_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}	/*	HI846PD2156_MIPI_RAW_SensorInit	*/
