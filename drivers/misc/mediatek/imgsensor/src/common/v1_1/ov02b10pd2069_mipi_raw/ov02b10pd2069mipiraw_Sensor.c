...
#define PFX "OV02B10PD2069_camera_sensor"
#define LOG_1 LOG_INF("OV02B10PD2069,MIPI 1LANE\n")
/***************************   Modify end    *******************************/
...
#define SIZE_1600_1200 0
#define SIZE_1600_900 1
static kal_uint32 size_mode;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV02B10PD2069_SENSOR_ID,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	...
	{1600, 1200, 0, 150, 1600, 900, 1600, 900, 0000, 0000, 1600, 900, 0, 0, 1600, 900},	/*custom1*/
};

/*hope add otp check start*/
static int vivo_otp_read_when_power_on;
extern int ov02b10pd2069_otp_read(void);
extern otp_error_code_t OV02B10PD2069_OTP_ERROR_CODE;
 MUINT32  sn_inf_main_ov02b10pd2069[13];  /*0 flag   1-12 data*/
 MUINT32  material_inf_main_ov02b10pd2069[4];
...
write_cmos_sensor(0x37,0x03);
write_cmos_sensor(0xfd,0x01);
write_cmos_sensor(0x92,0x10);
write_cmos_sensor(0x93,0x01);
write_cmos_sensor(0x90,0x1c);
write_cmos_sensor(0xfd,0x03);
...
{
	LOG_INF("B210425-1686 and B210531-932 preview_setting before E\n");
	sensor_init();
	LOG_INF("B210425-1686 and B210531-932 preview_setting after E\n");
	LOG_INF("E\n");
	/*
	 * @@Initial - MIPI 1-Lane 1600x1200 10-bit 30fps
	 * 100 99 1600 1200
	 */
	 size_mode = SIZE_1600_1200;
...
{
	LOG_INF("B210425-1686 and B210531-932 capture_setting before E\n");
	sensor_init();
	LOG_INF("B210425-1686 and B210531-932 capture_setting after E\n");
	LOG_INF("E\n");
	/*
	 * @@Initial - MIPI 1-Lane 1600x1200 10-bit 30fps
	 * 100 99 1600 1200
...
{
	LOG_INF("B210425-1686 and B210531-932 normal_video_setting before E\n");
	sensor_init();
	LOG_INF("B210425-1686 and B210531-932 normal_video_setting after E\n");
	LOG_INF("E\n");
	/*
	 * @@Initial - MIPI 1-Lane 1600x1200 10-bit 30fps
	 * 100 99 1600 1200
...
{
	LOG_INF("B210425-1686 and B210531-932 hs_video_setting before E\n");
	sensor_init();
	LOG_INF("B210425-1686 and B210531-932 hs_video_setting after E\n");
	LOG_INF("E\n");
	/*
	 * @@Initial - MIPI 1-Lane 1600x1200 10-bit 30fps
	 * 100 99 1600 1200
...
static void custom1_setting(void)
{
	...
	sensor_init();
	LOG_INF("B210425-1686 and B210531-932 hs_video_setting after E\n");
size_mode = SIZE_1600_900;
write_cmos_sensor(0xfd,0x00);  //;crop to 1600x900
write_cmos_sensor(0x4f,0x06);
write_cmos_sensor(0x50,0x40);  //;mipi hszie low 8bit
write_cmos_sensor(0x51,0x03);
write_cmos_sensor(0x52,0x84); // ;mipi vsize low 8bit
write_cmos_sensor(0xfd,0x01);
write_cmos_sensor(0x02,0x00);
write_cmos_sensor(0x03,0x70); // ;window hstart low 8bit
write_cmos_sensor(0x04,0x00);
write_cmos_sensor(0x05,0xA6); // ;window vstart low 8bit
write_cmos_sensor(0x06,0x03);
write_cmos_sensor(0x07,0x20); // ;window hsize low 8bit
write_cmos_sensor(0x08,0x03);
write_cmos_sensor(0x09,0x84); // ;window vsize low 8bit
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	kal_uint8 retry_cnt = retry_total;
		...
		kal_uint8 timeout = 0;
		timeout = check_i2c_timeout(0x02, imgsensor.i2c_write_id);
	    if (timeout) {
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				LOG_INF("ov02b10 start read eeprom ---vivo_otp_read_when_power_on = %d\n", vivo_otp_read_when_power_on);
				vivo_otp_read_when_power_on = ov02b10pd2069_otp_read();
				LOG_INF("ov02b10 read eeprom ---vivo_otp_read_when_power_on = %d,OV02B10PD2069_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, OV02B10PD2069_OTP_ERROR_CODE);
				/*vivo lxd add end*/
				return ERROR_NONE;
			}
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
	    printk("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  OV02B10PD2069_OTP_ERROR_CODE:%d \n", *feature_data,OV02B10PD2069_OTP_ERROR_CODE);
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = OV02B10PD2069_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_ov02b10pd2069, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main_ov02b10pd2069, sizeof(MUINT32)*4); 
				#if 0
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_ov02b10pd2069[%d]= 0x%x\n", i, sn_inf_main_ov02b10[i]);
						}
				#endif
			}
				break;
...
UINT32 OV02B10PD2069_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}				/*      OV02B10PD2069_MIPI_RAW_SensorInit      */
