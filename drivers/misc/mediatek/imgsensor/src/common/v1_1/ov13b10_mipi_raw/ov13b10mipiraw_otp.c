...
otp_error_code_t OV13B10_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
unsigned  int ov13b10_flag;
extern MUINT32  sn_inf_main_ov13b10[13];  /*0 flag   1-12 data*/
extern MUINT32  material_inf_main_ov13b10[4];  
...
int ov13b10_vivo_otp_read(void)
{
	int i = 0;
	...
	OV13B10_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	...
	if (vivo_otp_data[0x0A] != 0x11 ){
		OV13B10_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
		ov13b10_flag = 0;
		LOG_INF("VCM ID Error!!!    ov13b10_flag =%d\n", ov13b10_flag);
	}else{
		ov13b10_flag = 1;
		LOG_INF("ov13b10_flag = %d\n", ov13b10_flag);
	}
	...
	if (check_if_group_valid == 0x01) //all the data is valid
	{
		...
		{
			OV13B10_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("ModuleInfo_info error!!!    otp_error_code:%d\n",OV13B10_OTP_ERROR_CODE);
			return 0;
		} else if (((vivo_otp_data[0x02] != 0x03) ||(vivo_otp_data[0x08] != 0x42) ||(vivo_otp_data[0x07] != 0x0D) ||(vivo_otp_data[0x06] != 0x56)|| ((vivo_otp_data[0x09] != 0x11) && (vivo_otp_data[0x09] != 0x12)) || (vivo_otp_data[0x0A] != 0x11) || (vivo_otp_data[0x0B] != 0x05)) &&(vivo_otp_data[0x0B] != 0x11))
		{
			OV13B10_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
        ...
        {
			OV13B10_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
            LOG_INF("ModuleInfo_checksum error!!!   otp_error_code:%d\n",OV13B10_OTP_ERROR_CODE);
			return 0;
        }
		/****material info start****/
			material_inf_main_ov13b10[0] = (MUINT32)vivo_otp_data[0x10];
			material_inf_main_ov13b10[1] = (MUINT32)vivo_otp_data[0x11];
			material_inf_main_ov13b10[2] = (MUINT32)vivo_otp_data[0x12];
			material_inf_main_ov13b10[3] = (MUINT32)vivo_otp_data[0x13];
			LOG_INF("material_inf_main_ov13b10[0] = 0x%x, material_inf_main_ov13b10[1] = 0x%x, material_inf_main_ov13b10[2] = 0x%x,material_inf_main_ov13b10[3] = 0x%x\n",
				material_inf_main_ov13b10[0]  , material_inf_main_ov13b10[1],  material_inf_main_ov13b10[2], material_inf_main_ov13b10[3]);
		/****material info end****/
		//////fuseIDInfo_checksum
		checksum = 0;
		...
		sn_inf_main_ov13b10[0] = 0x01;
		for(i = 0;i < 12;i++){
			sn_inf_main_ov13b10[i +1] = (MUINT32)vivo_otp_data[i + SNInfo_addr+1];
			LOG_INF("sn_inf_main_ov13b10[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_main_ov13b10[i +1],  i +SNInfo_addr+1, vivo_otp_data[i + SNInfo_addr+1]);
		}
		//////AWB_checksum
		checksum = 0;
        ...
        if(vivo_otp_data[0x01] == 0x05){    
			...
			if (diff_inf_137 > 150 || diff_mac_137 > 150 || diff_inf_macro > 450 || diff_inf_macro < 150) {  /*AF code out of range*/
			    OV13B10_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
			    LOG_INF("QTech 2311137 AF out of range error!!!   otp_error_code:%d   VCM ID = 0x%x AF_inf = %d AF_mac = %d\n", OV13B10_OTP_ERROR_CODE, vivo_otp_data[0x0A], AF_inf, AF_mac);
			    return 0;
		    }
        }
		...
		OV13B10_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
