...
#define MMC_BLK_TIMEOUT_MS  (8 * 1000)        /* 8 seconds timeout */
			 ...
			 int type)
{
	...
	if (err != -EOPNOTSUPP) {
		...
		if (part_err) {
			 ...
			 */
			if (host->card && mmc_card_sd(host->card)) {
				pr_err("%s: %s removing bad card. set vivo bad_card=1\n", mmc_hostname(host), __func__);
				host->bad_card = 1;
...
{
	if (card && card->host && mmc_card_sd(card)) {
		printk("%s: %s removing bad card.cmd_abort.\n", mmc_hostname(card->host), __func__);
		card->host->bad_card = 1;
