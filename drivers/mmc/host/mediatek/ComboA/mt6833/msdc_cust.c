...
int msdc_of_parse(struct platform_device *pdev, struct mmc_host *mmc)
{
	...
	if (of_get_property(np, "vmc_vol_1800", NULL)) {
		mmc->vmc_vol_1800 = true;
		pr_notice("[msdc%d]: vmc_vol_1800(VMC 1.86V->1.8V)\n", mmc->index);
	} else
		mmc->vmc_vol_1800 = false;

	if (of_get_property(np, "fast_power_off", NULL)) {
		//char *version = get_board_version();
		//int i = 0;
		//for (int i = 0; i < 8; i++)
		//	pr_notice("[msdc%d]: board_info:byte=%d value=%c", mmc->index, i, *(version + i));
		//pr_notice("\n");
		if (of_get_property(np, "PD2069", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
		} else if (of_get_property(np, "PD2069F_EX", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
		} else if (of_get_property(np, "PD2102", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
		} else if (of_get_property(np, "PD2156", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
		} else if (of_get_property(np, "PD2159F_EX", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
		...
		mmc->fast_power_off = false;

	if (of_get_property(np, "support_sdr104", NULL)) {
		mmc->speed_mode_sdr104 = 1;
		pr_notice("[msdc%d]: support sdr104 mode", mmc->index);
	} else
		mmc->speed_mode_sdr104 = 0;
