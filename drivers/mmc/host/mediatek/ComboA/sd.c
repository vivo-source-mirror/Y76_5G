...
static void msdc_set_power_mode(struct msdc_host *host, u8 mode)
{
	...
	if (host->power_mode == MMC_POWER_OFF && mode != MMC_POWER_OFF) {
		...
		if (is_card_sdio(host) || (host->hw->flags & MSDC_SDIO_IRQ)) {
			...
			/* miaoqiang add Mark block_bad_card should not be set here.
			 * It is better to set it in function mmc_attach_sd() where real errors are found.
			 */
...
static int msdc_ops_get_cd(struct mmc_host *mmc)
{
	...
	/*vivo add: A bad card has been found. */
	if (host->mmc->bad_card) {
		printk(KERN_ERR "mmc%d: The Card has marked ad bad_card...\n", host->id);
	...
	printk(KERN_ERR
	"mmc%d:Card insert<%d> Block bad card<%d>, mrq<%p> claimed<%d> pwrcnt<%d> vivo bad_card<%d>\n",
		host->id,
		host->card_inserted,
		host->block_bad_card,
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	/* miaoqiang add for mmc->f_max has been set according to dts. */
	if (host->hw->host_function != MSDC_SD)
