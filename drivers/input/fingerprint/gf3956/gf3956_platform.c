...
static int gf3956_set_spi_status(struct gf_dev *gf_dev, int enable)
{
	int rc = 0;
	if (enable) {
...
int gf3956_request_resource(struct gf_dev *gf_dev, irq_handler_t thread_fn)
{
	...
	if (!gf_dev->resource_requested) {
		...
		/*register irq*/
		gf_dev->irq = gf3956_irq_num(gf_dev);
...
int gf3956_power_on(struct gf_dev *gf_dev)
{
    ...
    gf3956_set_spi_status (gf_dev, 1);
...
int gf3956_power_off(struct gf_dev *gf_dev)
{
	...
	gf3956_set_spi_status (gf_dev, 0);
