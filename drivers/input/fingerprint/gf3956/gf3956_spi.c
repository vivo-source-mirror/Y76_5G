...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_REQUEST_SEOURCE:
		...
		if (1 == support_soft_fpid) {
			retval = gf3956_request_resource(gf_dev, gf_irq);
		}
		break;
	case GF_IOC_RELEASE_SEOURCE:
		pr_info("%s: GF_IOC_RELEASE_SEOURCE ======\n", __func__);
		if (1 == support_soft_fpid) {
			gf3956_release_resource(gf_dev);
		}
		break;
	default:
...
#endif
{
	...
	if (0 == support_soft_fpid) {
		status = gf3956_request_resource(gf_dev, gf_irq);
		if (status) {
...
static int __init gf_init(void)
{
	...
	*/
	...
	if (is_atboot) {
		printk("%s, AT mode, return 0 \n", __func__);
		return 0;
	}

	struct device_node *node = of_find_compatible_node(NULL, NULL, "fp-id");
	if (node && of_property_read_bool(node, "vivo,support_soft_fingerprint_id")) {
		support_soft_fpid = 1;
	...
	if (0 == support_soft_fpid) {
		if (get_fp_id() != GOODIX_GF3658 && get_fp_id() != GOODIX_GF3626 && get_fp_id() != GOODIX_GF3636) {
			printk("%s(): wrong fp id, not gf3568 driver, exit\n", __func__);
			return 0;
		}
	}
#endif
