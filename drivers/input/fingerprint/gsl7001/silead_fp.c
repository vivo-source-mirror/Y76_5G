...
static
#endif
fp_debug_level_t sil_debug_level = ALL_LOG;//DBG_LOG;
...
#endif /* PROC_DIR */
		fp_dev->proc_entry = proc_create(PROC_NODE, 0600, fp_dev->proc_root, &silfp_proc_fops);
		if (fp_dev->proc_entry == NULL) {
...
silfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SIFP_IOC_ACQ_SPI:
		...
		silfp_set_default_spi_status(fp_dev, 1);
...
int silfp_dev_init(void)
{
	...
	if (os_boot_puresys == 1) {
		/*do not load gf driver */
		printk("%s:boot puresys, not load gsl7001 driver!\n", __func__);
		return 0;
	}
#endif
