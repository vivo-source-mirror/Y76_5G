...
static struct pm_qos_request gf_pm_qos_request;
static int gf_pm_qos_vcore_dvfs_opp_init(void)
{
	pm_qos_add_request(&gf_pm_qos_request, PM_QOS_DDR_OPP, PM_QOS_DDR_OPP_DEFAULT_VALUE);
	return 0;
}

static int gf_pm_qos_vcore_dvfs_opp_hold(void)
{
	pm_qos_update_request(&gf_pm_qos_request, PM_QOS_POWER_MODEL_DDR_REQUEST_DEFAULT_VALUE);
	return 0;
}

static int gf_pm_qos_vcore_dvfs_opp_release(void)
{
	pm_qos_update_request(&gf_pm_qos_request, PM_QOS_DDR_OPP_DEFAULT_VALUE);
	return 0;
}

static int gf_pm_qos_vcore_dvfs_opp_remove(void)
{
	pm_qos_remove_request(&gf_pm_qos_request);
	return 0;
}
...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	gf_dev->vdd_use_pmic = of_property_read_bool(node, "fp,vdd_use_pmic");
	gf_dev->vddio_use_gpio = of_property_read_bool(node, "fp,vddio_use_gpio");
	gf_dev->vddio_use_pmic = of_property_read_bool(node, "fp,vddio_use_pmic");

	gf_debug(DEBUG_LOG, "%s vdd_use_gpio %d\n", __func__, gf_dev->vdd_use_gpio);
	gf_debug(DEBUG_LOG, "%s vdd_use_pmic %d\n", __func__, gf_dev->vdd_use_pmic);
	gf_debug(DEBUG_LOG, "%s vddio_use_gpio %d\n", __func__, gf_dev->vddio_use_gpio);
	gf_debug(DEBUG_LOG, "%s vddio_use_pmic %d\n", __func__, gf_dev->vddio_use_pmic);
	...
	if (gf_dev->vdd_use_gpio) {
		...
		ret = gpio_request(gf_dev->vdd_en_gpio, "gpio_vdd_en");
		if (ret) {
	...
	if (gf_dev->vdd_use_pmic) {
		gf_dev->vdd = regulator_get(&gf_dev->spi->dev, "vfp");
	...
	if (gf_dev->vddio_use_gpio) {
		gf_dev->vddio_en_gpio = of_get_named_gpio(node, "fp,gpio_vddio_en", 0);
		...
		ret = gpio_request(gf_dev->vddio_en_gpio, "gpio_vddio_en");
		if (ret) {
			pr_info("%s Failed to request VDDIO_EN GPIO. ret = %d\n", __func__, ret);
			return -EIO;
		} else {
			pr_info("%s Success to request VDDIO_EN GPIO. ret = %d\n", __func__, ret);
		}
	}
...
static int gf_hw_get_power_state(struct gf_device *gf_dev)
{
	...
	if (gf_dev->vdd_use_gpio) {
		...
		retval = regulator_is_enabled(gf_dev->vdd);
	}
	return retval;
}
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	case GF_IOC_PM_QOS_REQUEST:
		gf_debug(ERR_LOG, "[%s]: GF_IOC_PM_QOS_REQUEST \n", __func__);
		retval = gf_pm_qos_vcore_dvfs_opp_hold();
		break;

	case GF_IOC_PM_QOS_RELEASE:
		gf_debug(ERR_LOG, "[%s]: GF_IOC_PM_QOS_RELEASE \n", __func__);
		retval = gf_pm_qos_vcore_dvfs_opp_release();
		break;
	default:
...
static int gf_probe(struct spi_device *spi)
{
	...
	if (status) {
		gf_debug(ERR_LOG, "%s, Failed to enable power.\n", __func__);
		goto err_reg;
	}
...
/* power enable end */
	...
	gf_spi_clk_enable(gf_dev, 0);
	/* init fingerprint pm qos */
	gf_pm_qos_vcore_dvfs_opp_init();
	FUNC_EXIT();
	return 0;
...
static int gf_remove(struct spi_device *spi)
{
	...
	/* remove fingerprint pm qos */
	gf_pm_qos_vcore_dvfs_opp_remove();
	kfree(gf_dev);
	FUNC_EXIT();
	return 0;
}
...
static int __init gf_init(void)
{
	...
	if (1 == is_support_soft_fpid()) {
		pr_err("%s, support software detect fpid, return.", __func__);
		return -EINVAL;
	}
