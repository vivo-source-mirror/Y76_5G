...
//zxd add for fan53870 reset if i2c can not work
...
static void fan53870_software_reset(struct regmap *regmap)
{

	int rc = 0;
	u8 sof_reset = 0xB6;

	pr_info("Doing software reset fan53870\n");

	rc = fan53870_write(regmap, 0x11, &sof_reset, 1);
	if (rc < 0) {
		pr_err("failed to write sof_reset\n");
	}else{
		pr_info("fan53870_software_reset success");
	}
...
#if defined(CONFIG_MTK_CAM_PD2159F_EX)			
			//zxd add for fan53870 reset beg
			reset_fan53870.reset_gpio = chip->gpio_reset;
			//zxd add for fan53870 reset end
#endif			
				    ...
				    const struct i2c_device_id *id)
{
	...
	if (rc == -EBUSY) {
		...
		//zxd add for pmic compatilbe beg
		return rc;
		//zxd add for pmic compatilbe end
	}
#else
	if (rc < 0) {
		pr_err("FAN53870: Unable to initialize GPIO\n");
	...
	fan53870_software_reset(regmap);
