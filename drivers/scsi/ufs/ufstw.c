...
bool ufstw_need_flush(struct ufsf_feature *ufsf)
{
	...
	if (ufsf_query_attr_retry(hba, UPIU_QUERY_OPCODE_READ_ATTR,
		...
		TW_DEBUG(ufsf, "ERROR: get available tw current buffer size error");
		/* some device may not support QUERY_ATTR_CUR_TW_BUF_SIZE, just skip check */
		goto skip_check;
	}

	if (tw->tw_current_tw_buffer_size <= 1024) {
		/* enable h8 flush when low space */
		ufsf_query_flag_retry(hba, UPIU_QUERY_OPCODE_SET_FLAG,
						QUERY_FLAG_IDN_TW_FLUSH_DURING_HIBERN,
						idx, NULL);
		tw->tw_flush_during_hibern_enter = true;

		TW_DEBUG(ufsf, "force enable h8 flush available_buffer:%d, current_buffer:%d",
			tw->tw_available_buffer_size,
			tw->tw_current_tw_buffer_size);
		goto out_put;
	}
...
skip_check:
	/* No need flush:writebooster is 10, turbo write is 0x10 */
	if (tw->tw_available_buffer_size == 10 || tw->tw_available_buffer_size == 0x10) {
		/* disable h8 flush */
		ufsf_query_flag_retry(hba, UPIU_QUERY_OPCODE_CLEAR_FLAG,
						QUERY_FLAG_IDN_TW_FLUSH_DURING_HIBERN,
						idx, NULL);
		tw->tw_flush_during_hibern_enter = false;

		TW_DEBUG(ufsf, "disable h8 flush available_buffer:%d, current_buffer:%d",
			tw->tw_available_buffer_size,
			tw->tw_current_tw_buffer_size);
	} else if (tw->tw_available_buffer_size < 10 &&
		tw->tw_available_buffer_size > 5) {
		/* do nothing */
		TW_DEBUG(ufsf, "do nothing h8 flush available_buffer:%d, current_buffer:%d",
			tw->tw_available_buffer_size,
			tw->tw_current_tw_buffer_size);
	} else {
		/* enable h8 flush */
		...
		TW_DEBUG(ufsf, "enable h8 flush available_buffer:%d, current_buffer:%d",
			tw->tw_available_buffer_size,
			tw->tw_current_tw_buffer_size);
	}
#if 0
