...
#ifdef CONFIG_RMS_ION_STAT
static struct ion_stat *find_list(struct list_head *head, pid_t pid)
{
	struct ion_stat *stat;
...
static int ion_proc_stat_show(struct seq_file *s, void *unused)
{
	struct ion_device *dev = g_ion_device;
	struct rb_node *n, *m;
		...
		{
			{
				...
				for (m = rb_first(&client->handles); m; m = rb_next(m)) {
					...
					if (handle->buffer->heap->id == ION_HEAP_TYPE_MULTIMEDIA ||
						...
						if ((handle->buffer->handle_count) != 0) {
							buffer_size += (int)(handle->buffer->size) / (handle->buffer->handle_count);
						} else {
							orphaned_size += (int)(handle->buffer->size);
						}
					}
				}
				if (!buffer_size) {
				...
				total_all_buffer += buffer_size;
				total_orphaned_all_buffer += orphaned_size;
				if (client->pid > 0) {
					stat = find_list(&pid_list, client->pid);
					if (!stat) {
						...
						if (stat) {
							...
							stat->size = 0;
							stat->orph_size = 0;
							list_add(&stat->list, &pid_list);
						}
					}
					if (stat) {
						stat->size += buffer_size;
						stat->orph_size += orphaned_size;
					}
				}
				buffer_size = 0;
				orphaned_size = 0;
	...
	list_for_each_entry_safe(stat, tmp, &pid_list, list) {
		seq_printf(s, " %d %zu %zu", stat->pid, stat->size, stat->orph_size);
		list_del(&stat->list);
		kfree(stat);
	}
	up_read(&dev->lock);
...
static int ion_proc_stat_open(struct inode *inode, struct file *file)
{
	return single_open(file, ion_proc_stat_show, inode->i_private);
}

static const struct file_operations ion_proc_stat_ops = {
	.open = ion_proc_stat_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
#ifdef CONFIG_RMS_ION_STAT
	g_ion_device->proc_entry = proc_create("meminfo_ion", 0444, NULL, &ion_proc_stat_ops);
	if (!g_ion_device->proc_entry)
